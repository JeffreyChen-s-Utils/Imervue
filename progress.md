...
- **#26** 4 broad `except Exception` handlers still neither log nor re-raise, each silenced with `# noqa: BLE001`: the `safety_review` plugin (4: `_class_config.py` ×2, `_detection.py` ×2). Next step: per package, catch what the block can meet (`IMAGE_READ_ERRORS`, `GLError`, `sqlite3.Error`, `OSError`) or log with `exc_info`; then extend `tests/test_no_silent_suppress.py` to reject a broad handler whose body neither logs nor raises.
