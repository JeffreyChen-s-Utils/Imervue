...
- **#24** 31 functions in `Imervue/` take more than the workspace's 7 parameters: `paint/` (16), `gpu_image_view/` (5), `gui/` (5), `image/` (2), `mcp_server/` (2), `library/` (1). The widest are `gui/image_sanitize_dialog.py:265` (11), `paint/manga_effects.py:51` (11) and `paint/speech_bubbles.py:41` (11); `py -m ruff check Imervue --select PLR0913 --config "lint.pylint.max-args=7"` lists them. `PLR0913` is in the ruff ignore list (`pyproject.toml`). Next step: group related options into a frozen dataclass per function (characterisation tests first), one package per commit; then set `pylint.max-args = 7` and drop `PLR0913` from the ignore list.
