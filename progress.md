...
Cross-repo and workspace items live in `D:\Codes\progress.md` (relevant here: S-10, X-9, X-10, X-18, W-4).

## Open

- **#1** `.idea/deployment.xml` is tracked and names an SFTP server as user@IP:port. Remove it from version control (workspace S-10, W-4).
