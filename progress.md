...
- **#26** 9 broad `except Exception` handlers still neither log nor re-raise, each silenced with `# noqa: BLE001`: `paint/canvas.py` (2), `mcp_server/tools_read.py` (2), `system/trash_ops.py`, and the `safety_review` plugin (4). Next step: per package, catch what the block can meet (`IMAGE_READ_ERRORS`, `GLError`, `sqlite3.Error`, `OSError`) or log with `exc_info`; then extend `tests/test_no_silent_suppress.py` to reject a broad handler whose body neither logs nor raises.
