...
- **#19** 9 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `puppet/cubism_native_bridge.py:_bind_signatures` (89), `paint/stamp_tool.py:stamp_dab` (86), `puppet/workspace.py:__init__` (84), `paint/gpu_brush.py:stamp` (84) and `puppet/canvas.py:render_offscreen_puppet` (83). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
...
- **#23** Five modules are within 50 lines of the 1,000-line limit: `plugin/pip_installer.py` (993), `desktop_pet/pet_window.py` (985), `Imervue_main_window.py` (971), `gpu_image_view/overlay_painter.py` (963), `gui/annotation_dialog.py` (956). Next step: move one cohesive group of methods out of each into its own module (characterisation tests first), largest first.
