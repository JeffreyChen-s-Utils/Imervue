...
- **#27** `py -m ruff check .` honours `.gitignore`, and `/plugins/` is gitignored (force-added), so the Definition-of-Done lint gate has never checked the bundled plugins; `py -m ruff check plugins/` reports 13 findings (8 `BLE001`, 2 `PLW2901`, `SIM115`, `UP015`, `UP037`). Bandit's gate covers only `Imervue/`. Next step: fix the findings, then make both gates include `plugins/` (config or the documented commands).
