...
- **#19** 21 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `puppet/canvas.py:__init__` (104), `paint/docks/brushes.py:FillDock.__init__` (99), `gui/gif_video_dialog.py:_build_ui` (97), `desktop_pet/pet_workspace.py:_build_window_group` (95) and `gui/smart_albums_dialog.py:__init__` (95). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
