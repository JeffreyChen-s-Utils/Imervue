...
- **#21** Every broad `except Exception` in `Imervue/` now narrows the type or logs the traceback, but ruff's `BLE` rules are still not in `[tool.ruff.lint] select` (`pyproject.toml`), so nothing stops a new one. Next step: add `BLE`; mark `tests/test_paint_canvas.py:113` with a justified `# noqa: BLE001`; and turn the 28 `# noqa: BLE001` that suppress nothing (`ruff check . --extend-select BLE,RUF100` lists them) into plain comments.
