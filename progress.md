...
- **#19** 34 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `paint/docks/layers.py:__init__` (146), `gui/annotation_dialog.py:_build_right_panel` (142), `gui/image_organizer_dialog.py:_build_ui` (142), `gui/ai_upscale_dialog.py:_build_ui` (141) and `paint/canvas.py:__init__` (133). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
