...
- **#19** 18 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `desktop_pet/pet_workspace.py:_build_window_group` (95), `gui/smart_albums_dialog.py:__init__` (95), `puppet/cubism_native_bridge.py:_bind_signatures` (89), `menu/filter_menu.py:build_filter_menu` (87) and `gui/slideshow_mp4_dialog.py:__init__` (86). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
