...
- **#17** `plugins/safety_review/safety_review.py` 1.0.1 (U-20260922-15: finds the shared "AI Tools" submenu with `findChildren`) is not mirrored: `D:\Codes\Imervue_Plugins` `main` still has 1.0.0 of that file. Copy it there, push `main`, and run the parity check in `CLAUDE.md` (Mirror plugin changes).
- **#18** `tests/test_face_detection.py` fails twice in the local `.venv` (`TestDetectFacesOutput` blank/noise image: `module 'cv2' has no attribute 'CascadeClassifier'`, `Imervue/image/face_detection.py:62`). The venv has three OpenCV distributions and `cv2` resolves to `opencv-python-headless` 5.0.0.93, next to `opencv-python` and `opencv-contrib-python` 4.13.0.92. Find what pulled in 5.0, then either fix the venv or make face detection handle OpenCV 5 (and pin it if needed).
