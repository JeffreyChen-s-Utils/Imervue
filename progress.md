...
- **#19** 25 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `paint/manga_menu.py:__init__` (112 and 108, two classes), `gpu_image_view/actions/compare_dialog.py:__init__` (110), `gui/exif_sidebar.py:__init__` (106) and `puppet/canvas.py:__init__` (104). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
