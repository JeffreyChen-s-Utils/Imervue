...
- **#19** 28 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `gui/batch_export_dialog.py:_build_ui` (123), `gui/annotation_dialog.py:__init__` (118), `paint/manga_menu.py:__init__` (112), `gpu_image_view/actions/compare_dialog.py:__init__` (110) and `gui/exif_sidebar.py:__init__` (106). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
