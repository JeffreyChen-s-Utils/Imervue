...
- **#19** 23 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `gpu_image_view/actions/compare_dialog.py:__init__` (110), `gui/exif_sidebar.py:__init__` (106), `puppet/canvas.py:__init__` (104), `paint/docks/brushes.py:FillDock.__init__` (99) and `gui/gif_video_dialog.py:_build_ui` (97). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
