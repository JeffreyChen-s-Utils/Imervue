...
- **#19** 31 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `gui/ai_upscale_dialog.py:_build_ui` (141), `paint/canvas.py:__init__` (133), `gui/batch_convert_dialog.py:_build_ui` (124), `gui/batch_export_dialog.py:_build_ui` (123) and `gui/annotation_dialog.py:__init__` (118). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
