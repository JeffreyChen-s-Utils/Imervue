...
- **#19** 38 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `menu/file_menu.py:build_file_menu` (200), `gui/image_sanitize_dialog.py:_build_ui` (153), `puppet/workspace_menus.py:_build_actions` (149) and the `__init__` of `paint/docks/layers.py` and `paint/docks/brushes.py` (146 each). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
