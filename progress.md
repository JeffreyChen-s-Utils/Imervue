...
- **#19** 11 functions in `Imervue/` are longer than the workspace's 80-line limit, mostly Qt `__init__` / `_build_ui` builders; the largest are `puppet/cubism_native_bridge.py:_bind_signatures` (89), `paint/stamp_tool.py:stamp_dab` (86), `puppet/workspace.py:__init__` (84), `paint/gpu_brush.py:stamp` (84) and `Imervue_main_window.py:closeEvent` (84). Next step: split each into section builders in the original order (a builder returns the widget and the caller adds it to its layout), one module per commit.
