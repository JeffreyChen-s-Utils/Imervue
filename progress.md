...
- **#20** 7 dialogs in `Imervue/gui/` still hand-build the path edit + Browse row that `gui/dialog_rows.py` provides (`folder_picker_row` / `path_browse_row`, with `save_path_into` / `open_path_into` for the picker): `focus_stack`, `hdr_merge`, `panorama`, `stack_blend` (edit and button placed separately in the layout); `export_dialog`, `gif_video_dialog` (unlabelled rows); `anaglyph_dialog` (unlabelled, picks the right-eye image); `lut_dialog` (extra Clear button). Next step: pin each row with a test against the current code (as `tests/test_output_path_rows.py` does), then convert one group per commit.
