...
def test_write_text_atomically_writes_utf8(tmp_path):
    from Imervue.system.atomic_write import write_text_atomically
    path = tmp_path / "a.json"
    path.write_text("old", encoding="utf-8")
    write_text_atomically(path, "臺北 ✓")
    assert path.read_text(encoding="utf-8") == "臺北 ✓"
    assert [p.name for p in tmp_path.iterdir()] == ["a.json"]


def test_write_text_atomically_keeps_the_old_text_when_the_swap_fails(tmp_path, monkeypatch):
    ...
    with pytest.raises(PermissionError):
        atomic_write.write_text_atomically(path, "new")
