...
def test_reset_view_restores_defaults(qapp):
    ...
    try:
        ...
        label.reset_view()
        assert (label._zoom, label._pan_x, label._pan_y) == (1.0, 0.0, 0.0)
    finally:
        label.deleteLater()
...
def test_wheel_zoom_emits_view_changed_and_clamps(qapp):
    ...
    try:
        ...
        for _ in range(40):
            label.wheelEvent(_wheel_event(label, 120))
        assert label._zoom == 12.0

        for _ in range(80):
            label.wheelEvent(_wheel_event(label, -120))
        assert label._zoom == 1.0
    finally:
        label.deleteLater()
...
def test_sync_view_applies_state_without_re_emitting(qapp):
    ...
    try:
        ...
        assert (label._zoom, label._pan_x, label._pan_y) == (2.0, 4.0, -3.0)
        assert emitted == []
    finally:
        label.deleteLater()
...
def test_sync_sbs_labels_propagates_to_other_labels_only(qapp):
    ...
    try:
        ...
        assert (other._zoom, other._pan_x, other._pan_y) == (5.0, 7.0, 9.0)
        assert source._zoom == 5.0
    finally:
        source.deleteLater()
        other.deleteLater()
