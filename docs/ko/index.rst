...
다른 프로그램이 이미지를 덮어쓰면(외부 편집기가 직접 덮어쓰든, 사본을 쓴 뒤 이름을 바꿔 교체하든) 뷰어가 새 버전을 보여 줍니다. 딥 줌으로 열린 이미지는 마지막 쓰기 후 0.5초 뒤(Imervue가 다시 앞으로 올 때도 다시 확인), 그리드 썸네일은 몇 초 안에 갱신됩니다.
