...
Windows에서 숨김으로 표시된 파일(탐색기와 폴더 트리에서도 숨겨짐)과 점으로 시작하는 이름(macOS가 메모리 카드나 네트워크 드라이브의 사진마다 옆에 쓰는 ``._photo.jpg`` 등)은 썸네일 그리드, 폴더 아이콘, 일괄 도구의 폴더 목록, 감시 폴더, 라이브러리 스캔, CLI에서 제외됩니다. 재귀 스캔은 ``$RECYCLE.BIN``\ 이나 Mac의 ``.Trashes`` 같은 숨김 폴더에 들어가지 않습니다. 일부러 연 숨김 이미지는 그대로 열립니다.
