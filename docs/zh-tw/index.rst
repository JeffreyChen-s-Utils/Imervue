...
.. list-table::
   ...
   * - **Paint**
     - MediBang 風格的點陣繪圖工作室，含筆刷、圖層、動畫、漫畫工具、PSD I/O。見「Paint 分頁 — MediBang 風格繪圖」。
   * - **Puppet**
...
第三個頂層分頁 — **繪圖** — 是 MediBang 風格的繪圖工作區，支援多分頁文件、
向量與點陣圖層、漫畫工具、動畫影格，以及 PSD 匯入/匯出。從分頁列切換進入，
或在大圖模式下按 ``E`` 將目前圖片送入新繪圖分頁。

UX 體質升級：Medibang 風格、會跟著 zoom 縮放的筆刷大小游標、每個工具獨立
...
``Extra Tools`` > ``Workflow`` > ``Dual-Pane File Manager`` 提供 Total Commander 風格的雙樹
