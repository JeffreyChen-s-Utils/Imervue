...
Вкладка 5 — **Desktop Pet** — запускает любой ``.puppet``-риг как
безрамочный, прозрачный, всегда поверх всех окон оверлей на вашем
рабочем столе. Внутри-приложенческая вкладка является панелью
управления; сам персонаж живёт в отдельном окне верхнего уровня,
которое разделяет среду выполнения Puppet (тот же
:class:`PuppetCanvas`, тот же конвейер параметров / движений /
физики, те же драйверы живого ввода).
...
.. list-table::
   ...
   * - Возможность
     - Примечания
   * - Безрамочный оверлей
     - Без оконного хрома, без записи в панели задач; сидит поверх
       всех остальных окон через ``Qt.WindowStaysOnTopHint``.
   * - Прозрачный фон
     - ``WA_TranslucentBackground`` + формат GL-поверхности с
       поддержкой альфа + ``glClearColor(0,0,0,0)`` — каждый
       пиксель, который марионетка не рисует, показывает рабочий
       стол сквозь.
   * - Перетаскивание для перемещения
     - Левое перетаскивание персонажа для смены позиции. Отпустите
       в пределах настраиваемого порога прилипания (по умолчанию
       24 пикс) от края экрана, чтобы **прилипнуть** заподлицо к
       нему. Быстрое перетаскивание с проскоком фиксируется обратно
       внутрь, так что питомец никогда не теряется за пределами
       экрана.
   * - Переключатель сквозного клика
     - Опциональный режим ``Qt.WindowTransparentForInput`` — каждый
       клик проходит сквозь к рабочему столу / приложению за
       питомцем.
   * - Фиксация якоря
     - Однокнопочная заморозка позиции питомца, чтобы случайные
       перетаскивания не могли его сдвинуть.
   * - Всегда внизу
     - Переключение с ``WindowStaysOnTopHint`` на
       ``WindowStaysOnBottomHint``, так что питомец сидит позади
       каждого окна как виджет рабочего стола (в паре с
       ``WindowDoesNotAcceptFocus``).
   * - Скрывать при полноэкранном режиме
     - Поллер 1 Гц следит за активным окном через Win32 API
       ``GetWindowRect`` (Windows) и автоматически скрывает
       питомца, пока другое приложение удерживает полноэкранный
       режим на мониторе питомца.
   * - Пауза при скрытии
     - Тик отрисовки 33 мс останавливается, пока оверлей скрыт,
       так что спящий питомец платит нулевой CPU. Возобновляется
       по ``showEvent``.
   * - Предустановки размера
     - Маленький (200×300) / средний (320×480) / большой (480×720);
       якорь по центру, так что питомец не прыгает по экрану при
       изменении размера.
   * - Слайдер непрозрачности
     - Непрозрачность уровня окна 0,1 – 1,0 через
       ``setWindowOpacity`` — композиция
       ``WA_TranslucentBackground`` плюс альфа на каждое окно даёт
       плавное затухание, а не просто затемнение пикселей
       марионетки.
   * - Сохранение позиции
     - Координаты ``(x, y)`` после прилипания, полученные после
       каждого отпускания перетаскивания, записываются в
       ``user_setting_dict["desktop_pet"]["position"]``. При
       следующем запуске питомец возвращается к этой позиции на
       экране; при отключении одного из нескольких мониторов
       откатывается к правому нижнему углу основного экрана.
...
.. list-table::
   ...
   * - Действие
     - Поведение
   * - **Левый клик по телу**
     - Отображает клик в координаты холста марионетки через
       обратную матрицу панорамирования / масштабирования,
       запускает существующий :func:`hit_test` против записей
       :class:`HitArea` документа и проигрывает связанное движение,
       если какое-либо покрывает целевой drawable. Откатывается к
       циклическому приветствию в речевом пузыре, когда ничего не
       совпадает.
   * - **Правый клик в любом месте**
     - Открывает контекстное меню с: Скрыть питомца, подменю
       **Live drivers** (6 включаемых переключателей), подменю
       **Play motion** (заполняется из ``document.motions``),
       подменю **Apply expression** (заполняется из
       ``document.expressions``), Зафиксировать позицию,
       Сквозной клик, Всегда внизу, Скрывать при полноэкранном
       режиме, переключатель речевого пузыря и подменю **Size**.
   * - **Речевой пузырь**
     - Безрамочный / прозрачный / всегда поверх виджет со
       скруглённым телом + хвостиком. Всплывает над питомцем при
       клике, держится ~4 с, затем угасает за 400 мс. Привязывается
       к геометрии питомца, так что перетаскивание питомца уводит
       пузырь с собой.
   * - **Системный трей**
     - Показать / Скрыть (с галочкой), Сквозной клик, Open puppet…,
       Скрыть питомца. Левый клик переключает видимость; правый
       клик открывает меню. Зеркалирует состояние галочек рабочего
       пространства через ``sync_visibility`` /
       ``sync_click_through``.

Живые драйверы (ленивая инициализация)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Каждый драйвер инстанцируется при первом включении, так что спящий
питомец платит нулевую стоимость таймеров / потоков:

* **Auto idle** — дыхание + дрейф на стандартных параметрах
  (``ParamBreath`` …) через :class:`IdleDriver`.
* **Idle motions** — случайный цикл по движениям группы ``Idle``
  через :class:`IdleMotionCycler` + поставляемый
  :class:`MotionPlayer`.
* **Auto-blink** — косинусоидальный цикл закрытие-открытие каждые
  ~4,5 с на ``ParamEyeLOpen`` / ``ParamEyeROpen`` через
  ``InputEngine.set_blink_enabled``.
* **Drag-track head** — смещение курсора → ``ParamAngleX/Y`` +
  ``ParamEyeBallX/Y`` через ``InputEngine.set_drag_enabled``.
* **Mic lip-sync** — RMS микрофона → ``ParamMouthOpenY`` через
  ``InputEngine.set_lipsync_enabled`` (требуется ``sounddevice``).
* **Webcam tracking** — MediaPipe FaceLandmarker → голова + глаза +
  рот через :class:`WebcamTracker` (требуются ``opencv-python`` +
  ``mediapipe``).

Сохранение состояния
^^^^^^^^^^^^^^^^^^^^

:mod:`Imervue.desktop_pet.settings` накладывается поверх
``user_setting_dict["desktop_pet"]`` с:

* Значениями по умолчанию для каждого ключа + ограничением
  диапазонов при загрузке, так что повреждённый файл настроек не
  может уронить запуск.
* Слиянием на один уровень в глубину — более старые файлы настроек
  без новых ключей всё равно дают полный словарь состояния.
* Прямой совместимостью для подсловаря ``drivers`` — неизвестные
  ключи драйверов проходят туда-обратно нетронутыми, так что
  будущая версия, добавляющая новый драйвер, может чисто читать
  существующие файлы.

Каждая настраиваемая пользователем поверхность (позиция, размер,
непрозрачность, сквозной клик, якорь, на дне, скрытие при
полноэкранном режиме, речь, порог прилипания, каждый драйвер,
последний загруженный риг, показ при запуске) проходит туда-обратно
через этот хелпер, так что питомец возвращается в то же состояние
...
.. list-table::
   ...
   * - ``Imervue/desktop_pet/pet_window.py``
     - Оверлей верхнего уровня — безрамочный / всегда поверх /
       ``WA_TranslucentBackground``. Размещает
       ``PuppetCanvas(pet_mode=True)``, владеет перетаскиванием для
       перемещения, hit-detect, контекстным меню, интеграцией
       речевого пузыря, подключением детектора полноэкранного
       режима, драйверами, сквозной записью сохранения состояния.
   * - ``Imervue/desktop_pet/edge_snap.py``
     - Чистая Python-математика прилипания (без Qt) для юнит-
       тестируемой стыковки по углам / краям + фиксации проскока.
   * - ``Imervue/desktop_pet/settings.py``
     - Хелпер сохранения состояния — load / save / update / clamp.
   * - ``Imervue/desktop_pet/speech_bubble.py``
     - Безрамочный оверлей со скруглённым пузырём с
       позиционированием по якорю-прямоугольнику + анимацией
       затухания.
   * - ``Imervue/desktop_pet/fullscreen_detector.py``
     - Цикл опроса 1 Гц, читающий прямоугольник окна на переднем
       плане (Win32 ctypes на Windows; no-op запасной вариант на
       других ОС) и эмитящий ``state_changed(bool)``.
   * - ``Imervue/desktop_pet/pet_workspace.py``
     - Вкладка панели управления. Лениво создаёт оверлей,
       экспонирует каждый переключатель / слайдер / комбобокс как
       чекбокс или спинбокс, сохраняет последний загруженный риг +
       поведение показа при запуске.
   * - ``Imervue/desktop_pet/tray_icon.py``
     - Хелпер системного трея — одна копия на сессию,
       синхронизируется с состоянием галочек рабочего
       пространства.

``PuppetCanvas.__init__(pet_mode=True)`` короткозамыкает фон шахматной
прозрачности редактора и оверлей выделения; остальная часть пути
рендеринга (VBO мешей, плеер движений, физика, выражения, группы
поз) идентична вкладке Puppet.

Каждая строка UI маршрутизируется через
``language_wrapper.language_word_dict.get(...)`` с ключами,
определёнными во всех пяти базовых языковых пакетах (English,
繁體中文, 简体中文, 日本語, 한국어).
