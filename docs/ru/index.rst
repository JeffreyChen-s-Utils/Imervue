...
остальные теги и миниатюра остаются прежними. Для других форматов нужен ``piexif``.

**Редактор EXIF** (кнопка ``Edit EXIF`` на боковой панели EXIF) меняет описание, автора, авторские права, производителя / модель камеры и комментарий. Для JPEG не нужны дополнительные пакеты, переписывается только блок EXIF; для WebP нужен ``piexif``, а для других форматов показывается, почему их нельзя изменить.
