...
Когда другая программа сохраняет поверх изображения — внешний редактор, напрямую или переименовывая копию поверх, — просмотрщик показывает новую версию: изображение, открытое в глубоком зуме, — через полсекунды после последней записи, миниатюры сетки — в течение нескольких секунд.
