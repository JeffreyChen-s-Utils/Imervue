...
вкладка — это панель управления; персонаж парит поверх (или позади)
ваших остальных окон. Всё, что можно делать с ригом во вкладке
Puppet — движения, выражения, физика, idle-драйверы, ввод с веб-
камеры / микрофона — работает и здесь.
...
.. list-table::
   ...
   * - Возможность
     - Описание
   * - Безрамочный оверлей
     - Без оконного хрома, без записи в панели задач — только
       персонаж на рабочем столе.
   * - Прозрачный фон
     - Всё, что персонаж не закрывает, показывает рабочий стол
       сквозь себя.
   * - Перетаскивание для перемещения
     - Левое перетаскивание персонажа в новое место. Отпустите
       рядом с краем экрана, чтобы **прилипнуть** к нему
       заподлицо.
   * - Режим сквозного клика
     - Заставьте питомца игнорировать вашу мышь, чтобы вы могли
       продолжать работать под ним.
   * - Фиксация позиции
     - Заморозьте питомца, чтобы случайные перетаскивания не могли
       его сдвинуть.
   * - Всегда внизу
     - Посадите питомца позади всех остальных окон — ощущение
       виджета рабочего стола вместо «всегда поверх».
   * - Скрывать при полноэкранном режиме
     - Автоматически скрывается, пока другое приложение (игра /
       видео / презентация) находится в полноэкранном режиме на
       том же мониторе; возвращается, когда полноэкранный режим
       заканчивается.
   * - Пауза при скрытии
     - Питомец перестаёт анимироваться, пока невидим — нулевая
       нагрузка на CPU, когда он за экраном.
   * - Предустановки размера
     - Маленький / средний / большой. Изменение размера происходит
       вокруг центра, так что питомец не прыгает по экрану.
   * - Слайдер непрозрачности
     - Затухание питомца от 10 % до 100 %, чтобы он мог быть
       ненавязчивым украшением рабочего стола.
   * - Запоминает, куда вы его поставили
     - Перетащите питомца в любимый угол; при следующем запуске он
       вернётся туда же.

Взаимодействие кликом
^^^^^^^^^^^^^^^^^^^^^

* **Левый клик по телу** — если риг определяет область попадания
  (например, нажатие на голову), проигрывается соответствующее
  движение. Иначе питомец приветствует вас речевым пузырём.
* **Правый клик в любом месте** — открывает контекстное меню с:
  Скрыть питомца, Live drivers, Play motion (список всех движений
  в риге), Apply expression, Зафиксировать позицию, Сквозной клик,
  Всегда внизу, Скрывать при полноэкранном режиме, Речевой пузырь,
  Размер.
* **Иконка в системном трее** — левый клик переключает видимость,
  правый — Показать / Скрыть, Сквозной клик, Open puppet, Скрыть
  питомца.

Живые драйверы
^^^^^^^^^^^^^^

Выберите любую комбинацию из вкладки или контекстного меню. Каждый
выключен по умолчанию — включайте только то, что хотите.

* **Auto idle** — дыхание + лёгкий дрейф, чтобы персонаж казался
  живым.
* **Idle motions** — случайный цикл по движениям группы idle в
  риге.
* **Auto-blink** — естественный циклический закрытие-открытие глаз
  каждые несколько секунд.
* **Drag-track head** — голова поворачивается, следуя за курсором.
* **Mic lip-sync** — рот открывается под ваш голос (требуется
  ``sounddevice``).
* **Webcam tracking** — ваша голова / глаза / рот управляют
  марионеткой (требуются ``opencv-python`` и ``mediapipe``).
...
2. Нажмите **Load bundled March 7th**, чтобы использовать
   встроенного персонажа, или **Open Puppet…**, чтобы выбрать свой
   ``.puppet``-файл.
3. Отметьте **Show pet on desktop**.
4. Перетащите персонажа туда, где он вам нужен; выберите нужные
   драйверы; настройте непрозрачность / размер.
5. В любой момент правый клик откроет меню быстрых действий, или
   используйте иконку в системном трее, чтобы скрыть питомца, не
   возвращаясь к вкладке.

Всё, что вы настроили — позиция, драйверы, непрозрачность, сквозной
клик, размер — запоминается между запусками.
