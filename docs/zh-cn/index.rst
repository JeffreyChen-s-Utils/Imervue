...
.. list-table::
   ...
   * - **Paint**
     - MediBang 风格的栅格绘图工作室，含笔刷、图层、动画、漫画工具、PSD I/O。见"Paint 标签 — MediBang 风格绘图"。
   * - **Puppet**
...
第三个顶层标签 — **绘图** — 是一个 MediBang 风格的绘图工作区，支持多文档标签、
矢量与位图图层、漫画工具、动画帧，以及 PSD 导入/导出。从标签栏切换进入，或在
深度缩放模式下按 ``E`` 将当前图像直接送入新的绘图标签。

UX 体质升级：Medibang 风格、会跟着 zoom 缩放的笔刷大小光标、每个工具独立
...
``Extra Tools`` > ``Workflow`` > ``Dual-Pane File Manager`` 提供 Total Commander 风格的双
