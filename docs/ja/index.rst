...
ディスク上のファイルが変わるのは指示したときだけです。**Apply Crop** と注釈の **Save** は結果を元ファイルに書き戻しますが、EXIF（カメラ、撮影日時、GPS）、XMP、DPI は保持されます。カメラ RAW、HEIC、アニメーション / マルチページのファイルは決して上書きされず、切り抜きはエクスポートを促し、注釈の保存は新しいファイル名を尋ねます。
