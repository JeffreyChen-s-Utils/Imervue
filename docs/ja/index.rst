...
Windows で隠し属性の付いたファイル（エクスプローラーやフォルダーツリーでも非表示）と、ドットで始まる名前（macOS がメモリーカードやネットワークドライブで写真ごとに書く ``._photo.jpg`` など）は、サムネイル一覧、フォルダーアイコン、バッチツールのフォルダー一覧、監視フォルダー、ライブラリのスキャン、CLI から除外されます。再帰スキャンは ``$RECYCLE.BIN`` や Mac の ``.Trashes`` などの隠しフォルダーに入りません。意図して開いた隠し画像はそのまま開けます。
