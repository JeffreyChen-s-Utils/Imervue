...
タブ 5 — **Desktop Pet** は任意の ``.puppet`` rig を、フレームレス・透明・常に最前面のオーバーレイとしてデスクトップ上に表示します。アプリ内タブはコントロールパネルで、実際のキャラクター本体は Puppet ランタイム（同じ :class:`PuppetCanvas`、同じパラメータ / モーション / 物理パイプライン、同じライブ入力ドライバ）を共有する別のトップレベルウィンドウに表示されます。
...
.. list-table::
   ...
   * - 機能
     - 説明
   * - フレームレスオーバーレイ
     - ウィンドウ装飾なし、タスクバーエントリなし。``Qt.WindowStaysOnTopHint`` で他のすべてのウィンドウより前面に配置。
   * - 透明背景
     - ``WA_TranslucentBackground`` + アルファ対応 GL サーフェスフォーマット + ``glClearColor(0,0,0,0)`` — パペットが描画しないピクセルからは下のデスクトップが透けて見える。
   * - ドラッグで移動
     - キャラクター本体を左ドラッグで再配置。設定可能なスナップ閾値（デフォルト 24 px）内で画面端付近に離すと **スナップ** して端にぴったり吸着。高速ドラッグによるオーバーシュートも画面内にクランプされ、ペットが画面外に消えない。
   * - クリックスルー切替
     - オプションの ``Qt.WindowTransparentForInput`` モード — すべてのクリックがペットの背後のデスクトップ / アプリへ素通り。
   * - アンカーロック
     - ワンクリックで位置を固定し、誤ドラッグによる移動を防ぐ。
   * - 常に最背面
     - ``WindowStaysOnTopHint`` から ``WindowStaysOnBottomHint`` へ切替えると、ペットが全ウィンドウの背後に回ってデスクトップウィジェットとして座る（``WindowDoesNotAcceptFocus`` と併用）。
   * - フルスクリーン時に隠す
     - 1 Hz のポーラーが Win32 ``GetWindowRect`` API（Windows）でアクティブウィンドウを監視し、別アプリがペットのモニター上でフルスクリーンを保持している間は自動的にペットを隠す。
   * - 非表示時は一時停止
     - オーバーレイが非表示の間は 33 ms ペイントティックが停止し、休眠中のペットは CPU をゼロ消費。``showEvent`` で再開。
   * - サイズプリセット
     - 小（200×300）/ 中（320×480）/ 大（480×720）。中央アンカー固定なので、リサイズ時にペットが画面上を飛び回らない。
   * - 不透明度スライダー
     - ``setWindowOpacity`` によるウィンドウレベル不透明度 0.1 – 1.0 — ``WA_TranslucentBackground`` コンポジット + ウィンドウ単位のアルファで、単なる減光ではなく滑らかなフェードを実現。
   * - 位置の永続化
     - ドラッグリリース後のスナップ済み ``(x, y)`` を ``user_setting_dict["desktop_pet"]["position"]`` に書き込み。次回起動時にその画面位置へ復帰；マルチモニター切断時はプライマリスクリーン右下にフォールバック。
...
.. list-table::
   ...
   * - アクション
     - 挙動
   * - **本体を左クリック**
     - クリック位置をパン / ズーム逆行列でパペット canvas 座標へ変換し、既存の :func:`hit_test` を文書の :class:`HitArea` エントリに対して実行、ヒットした drawable をカバーするモーションがあれば再生。該当なしの場合は speech bubble にラウンドロビン挨拶をフォールバック表示。
   * - **どこでも右クリック**
     - コンテキストメニュー：Hide pet、**Live drivers** サブメニュー（チェック可能トグル 6 個）、**Play motion** サブメニュー（``document.motions`` から自動生成）、**Apply expression** サブメニュー（``document.expressions`` から自動生成）、Lock position、Click-through、Always on bottom、Hide-on-fullscreen、Speech bubble トグル、**Size** サブメニュー。
   * - **Speech bubble**
     - フレームレス・透明・常に最前面のウィジェット、丸みのある本体 + テール付き。クリック時にペット上へポップし、約 4 秒保持してから 400 ms かけてフェード。ペットのジオメトリにアンカーされるため、ペットをドラッグすると吹き出しも追従。
   * - **システムトレイ**
     - Show / Hide（チェック可能）、Click-through、Open puppet…、Hide pet。左クリックで表示切替、右クリックでメニュー。``sync_visibility`` / ``sync_click_through`` でワークスペースのチェック状態とミラー。

ライブドライバ（遅延初期化）
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

各ドライバは初回有効化時にインスタンス化されるため、休眠中のペットはタイマー / スレッドコストをゼロに保ちます：

* **Auto idle** — :class:`IdleDriver` 経由で標準パラメータ（``ParamBreath`` …）に呼吸 + ドリフト。
* **Idle motions** — :class:`IdleMotionCycler` + 同梱の :class:`MotionPlayer` で ``Idle`` グループのモーションをランダムサイクル。
* **Auto-blink** — ``InputEngine.set_blink_enabled`` 経由で約 4.5 秒ごとの cosine close-open を ``ParamEyeLOpen`` / ``ParamEyeROpen`` に。
* **Drag-track head** — ``InputEngine.set_drag_enabled`` 経由で、カーソルオフセット → ``ParamAngleX/Y`` + ``ParamEyeBallX/Y``。
* **Mic lip-sync** — ``InputEngine.set_lipsync_enabled`` 経由で、マイク RMS → ``ParamMouthOpenY``（``sounddevice`` 必要）。
* **Webcam tracking** — :class:`WebcamTracker` 経由で MediaPipe FaceLandmarker → 頭 + 目 + 口（``opencv-python`` + ``mediapipe`` 必要）。

永続化
^^^^^^

:mod:`Imervue.desktop_pet.settings` は ``user_setting_dict["desktop_pet"]`` の上に次のレイヤーを重ねます：

* 全キーのデフォルト値 + ロード時の範囲クランプ — 壊れた設定ファイルでも起動を落とさない。
* 1 階層深いマージ — 新キーが欠けた古い設定ファイルでも完全な状態 dict を生成。
* ``drivers`` サブ dict の前方互換 — 未知のドライバキーは無加工で往復するため、新ドライバを追加した将来バージョンも既存ファイルをクリーンに読める。

すべてのユーザー調整可能サーフェス（位置、サイズ、不透明度、クリックスルー、アンカー、最背面、フルスクリーン時非表示、speech、スナップ閾値、各ドライバ、最後にロードした rig、起動時表示）はこのヘルパー経由でラウンドトリップされ、次回起動時に同じ状態へ復帰します。
...
.. list-table::
   ...
   * - ``Imervue/desktop_pet/pet_window.py``
     - トップレベルオーバーレイ — フレームレス / 常に最前面 / ``WA_TranslucentBackground``。``PuppetCanvas(pet_mode=True)`` をホストし、ドラッグ移動、ヒット検出、コンテキストメニュー、speech bubble 連携、フルスクリーン検出配線、ドライバ、永続化のライトスルーを所有。
   * - ``Imervue/desktop_pet/edge_snap.py``
     - 純 Python のスナップ演算（Qt 非依存）。単体テスト可能なコーナー / エッジドッキング + オーバーシュートクランプ。
   * - ``Imervue/desktop_pet/settings.py``
     - 永続化ヘルパー — load / save / update / clamp。
   * - ``Imervue/desktop_pet/speech_bubble.py``
     - フレームレス丸みオーバーレイ吹き出し。矩形アンカー配置 + フェードアニメーション。
   * - ``Imervue/desktop_pet/fullscreen_detector.py``
     - 1 Hz ポーリングループでフォアグラウンドウィンドウの rect を読み（Windows は Win32 ctypes、それ以外は no-op フォールバック）、``state_changed(bool)`` を emit。
   * - ``Imervue/desktop_pet/pet_workspace.py``
     - コントロールパネルタブ本体。オーバーレイを遅延生成し、全トグル / スライダー / コンボをチェックボックスまたはスピンボックスとして公開、最後にロードした rig と起動時表示挙動を永続化。
   * - ``Imervue/desktop_pet/tray_icon.py``
     - システムトレイヘルパー — セッションあたり単一インスタンス、ワークスペースのチェック状態と同期。

``PuppetCanvas.__init__(pet_mode=True)`` はエディタの透明チェッカー背景と選択オーバーレイを短絡します；残りの描画パス（メッシュ VBO、モーションプレイヤー、物理、表情、ポーズグループ）は Puppet タブと同一です。

すべての UI 文字列は ``language_wrapper.language_word_dict.get(...)`` 経由でルーティングされ、キーは 5 つのベース言語パック（English、繁體中文、简体中文、日本語、한국어）すべてで定義されています。
