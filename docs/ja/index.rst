...
``Delete`` で選択した行を削除し、``Ctrl + Z`` で元に戻せます。評価（``0`` – ``5``）、カリング（``P`` / ``Shift + X`` / ``U``）、カラー（``F1`` – ``F5``）のキーも選択した行に付きます（グリッドと同じ）。キーはショートカット設定に従います。
...
Pillow の安全上限（1 億 7900 万画素）を大きく超えるパノラマも開けます。上限はパソコンのメモリ量に合わせて決まり（16 GB なら約 13 億画素）、こうした巨大な画像は 1 枚ずつデコードします。
...
- フォーマット選択：PNG、JPEG、WebP、BMP、TIFF、AVIF。``pillow-heif`` / ``pillow-jxl-plugin`` があれば HEIC / JPEG XL も
- 品質調整（非可逆フォーマットの場合）
...
の配置、スプリッターのサイズ、アクティブなルートフォルダを名前付きで保存し、保存済みレイアウトをワンクリックで切り替えられます。
ダイアログは現在の状態を保存、読み込み、リネーム、削除に対応しています。ワークスペースは
``user_settings.json``\ （``workspaces`` キー配下）に保存され、セッションをまたいで保持されます。

.. tip::
   ツリーとサムネイルグリッドを表示した **Browse** ワークスペースと、現像パネルを最大化して
   ツリーを折りたたんだ別の **Develop** ワークスペースを作成しておきましょう。ワンクリックで
