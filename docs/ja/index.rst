...
タブ 5 — **Desktop Pet** は、任意の ``.puppet`` キャラクターをフレームレス・透明のオーバーレイとしてデスクトップに表示します。タブ自体はコントロールパネルで、実際のキャラクターは他のウィンドウの前面（または背面）に浮かびます。Puppet タブで rig に対してできること — モーション、表情、物理、アイドルドライバ、ウェブカメラ / マイク入力 — はすべてここでも動作します。
...
.. list-table::
   ...
   * - 機能
     - 説明
   * - フレームレスオーバーレイ
     - ウィンドウ装飾もタスクバーエントリもなし — デスクトップ上にキャラクターだけ。
   * - 透明背景
     - キャラクターが覆っていない部分は、下のデスクトップがそのまま透けて見えます。
   * - ドラッグで移動
     - キャラクターを左ドラッグして好きな位置へ。画面端の近くで離すと、その端に **スナップ** してぴったり吸着します。
   * - クリックスルーモード
     - ペットがマウスを無視するようにして、その下で作業を続けられます。
   * - 位置のロック
     - ペットを固定し、誤ドラッグで動いてしまうのを防ぎます。
   * - 常に最背面
     - ペットを他のすべてのウィンドウの背後に座らせて、常に最前面ではなくデスクトップウィジェットのような感覚で使えます。
   * - フルスクリーン時は隠す
     - 同じモニターで別のアプリ（ゲーム / 動画 / プレゼン）がフルスクリーン中は自動的に隠れ、フルスクリーンが終わると戻ってきます。
   * - 非表示時は一時停止
     - 見えていない間はペットのアニメーションが止まる — オフスクリーン中は CPU 消費ゼロ。
   * - サイズプリセット
     - 小 / 中 / 大。中央を基準にリサイズされるので、サイズ変更でペットが画面上を飛び回りません。
   * - 不透明度スライダー
     - ペットを 10% から 100% までフェードでき、控えめなデスクトップ装飾としても使えます。
   * - 位置を記憶
     - 好きな隅にドラッグしておけば、次回起動時もそこへ戻ります。

クリック操作
^^^^^^^^^^^^

* **本体を左クリック** — rig にヒットエリアが定義されていれば（例えば頭をタップなど）、対応するモーションが再生されます。定義がない場合は、ペットが speech bubble で挨拶してくれます。
* **どこでも右クリック** — Hide pet、Live drivers、Play motion（rig 内の全モーション一覧）、Apply expression、Lock position、Click-through、Always on bottom、Hide on fullscreen、Speech bubble、Size を含むコンテキストメニューが開きます。
* **システムトレイアイコン** — 左クリックで表示切替、右クリックで Show / Hide、Click-through、Open puppet、Hide pet。

ライブドライバ
^^^^^^^^^^^^^^

タブまたは右クリックメニューから、好きな組み合わせを選べます。各ドライバは初期状態オフなので、使いたいものだけオンにしてください。

* **Auto idle** — 呼吸 + ささやかなドリフトでキャラクターが生きているように感じられます。
* **Idle motions** — rig のアイドルグループのモーションをランダムにサイクル再生します。
* **Auto-blink** — 数秒ごとに自然な周期で目を閉じます。
* **Drag-track head** — 頭がカーソルを追って向きを変えます。
* **Mic lip-sync** — 口があなたの声に合わせて開きます（``sounddevice`` が必要）。
* **Webcam tracking** — あなたの頭 / 目 / 口がパペットを動かします（``opencv-python`` と ``mediapipe`` が必要）。
...
4. キャラクターを好きな位置にドラッグし、使いたいドライバを選び、不透明度 / サイズを調整します。
5. クイックアクションメニューはいつでも右クリックで開けます。タブを探さずにペットを隠したいときはシステムトレイアイコンが便利です。

設定したすべて — 位置、ドライバ、不透明度、クリックスルー、サイズ — は次回起動時にも記憶されています。
