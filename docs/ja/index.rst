...
右側のドック（カラー、ブラシ、塗りつぶし、スウォッチ、レイヤー、ナビゲーター、
履歴、ページ、アニメーション、ヒストグラム、素材ライブラリ、スタンプ、
ポーズ、参考）は単一の列にタブ化され、
...
``Extra Tools`` > ``Semantic Search`` では、自然言語のフレーズ（例：*「雪の中のゴールデンレトリバー」*、*「夜のネオン街」*）を入力すると、開いているフォルダーの画像を関連度順に返します。各画像は CLIP のビジョン / 言語エンコーダーで埋め込み、パスと一緒に保存します。テキストクエリも同じベクトル空間に埋め込み、コサイン類似度で比較します。
