...
- **EXIF 편집기** 다이얼로그 — 설명·작성자·저작권·카메라·코멘트(유니코드 포함)를 추가 패키지 없이 JPEG에 기록하며 픽셀과 다른 태그는 그대로 (WebP는 `piexif` 필요)
...
- **GPS 지오태그 편집기** — EXIF GPS 위도/경도 읽기/쓰기. JPEG는 추가 패키지 없이 픽셀·다른 태그·썸네일을 그대로 두고 기록 (다른 형식은 `piexif` 필요)
