...
`Imervue.cli`는 **Qt를 시작하지 않고** 순수 이미지 연산을 셸에서 실행합니다. 스크립트, CI 단계, 디스플레이가 없는 서버에 유용합니다:
...
- **라이브러리 검색** — SQLite 다중 루트 인덱스를 파일명, 최소 너비 / 높이, 파일 크기로 검색 (최대 2000개 결과; 더블클릭하면 열림)
...
**Modify** 탭은 현상 작업장입니다. 모든 조정은 파일과 함께 보관되는 이미지별 **recipe**에 저장되며, **Export** 또는 **Save As**를 명시적으로 실행하기 전까지 디스크상의 원본 픽셀은 결코 덮어쓰이지 않습니다. 예외는 **Apply Crop**과 주석 **Save** 두 가지로, 결과를 원본 파일에 다시 쓰되 EXIF(카메라, 촬영 일시, GPS), XMP, DPI는 유지합니다. 카메라 RAW, HEIC, 애니메이션 / 다중 페이지 파일은 절대 덮어쓰지 않으며, 자르기는 내보내기를 안내하고 주석 저장은 새 파일을 묻습니다. 한 번 적용하는 도구(CLAHE, HSL 믹서, 사진 프레임, 자동 수평 보정 등)는 결과를 원본 옆에 `photo_clahe.png`로 저장하며, 다시 실행하면 이전 결과를 바꾸지 않고 `photo_clahe_1.png`로 저장합니다. **EXIF로 자동 회전**, **EXIF 일괄 제거**의 사본, **페이지 분할…**도 같은 방식으로 번호를 붙입니다. Imervue가 사진을 무손실 회전하거나(자르기 영역도 함께 회전) EXIF를 다시 쓰면(GPS 지오태그, EXIF 편집기) 사진의 레시피와 가상 사본이 함께 따라갑니다. 로컬 마스크, 레이어, 렌즈 플레어, 얼굴 태그가 있는 레시피는 회전 전 버전에 남으며 되돌리면 다시 쓸 수 있습니다.
...
- **솔라리제이션** — 암실 스타일 톤 반전 (임계값 + 믹스)
- **디퓨즈 글로우 / Orton** — 소프트 포커스 하이라이트 블룸 (양 / 반경 / 하이라이트 임계값)
- **그라데이션 맵** — 휘도 → 팔레트, 채도 높은 그라데이션이 중간점에서 회색으로 바래지 않고 선명함을 유지하는 선택적 지각적 (OkLCH) 보간 모드 포함
- **정렬 디더링** — Bayer 행렬 기반 N단계 양자화 (양극단 보존)
- **그라데이션 농도 (Graduated Density)** — 각도 / 경도 / 오프셋에 의한 선형 ND 그라데이션, 하늘과 전경을 위한 선택적 틴트 포함
- **톤 이퀄라이저 (Tone Equalizer)** — 평활화된 마스크 위에서 휘도 영역(그림자에서 하이라이트까지)별로 독립적인 노출 적용
- **디테일 이퀄라이저 (Detail Equalizer)** — 단일 클래리티 슬라이더를 넘어, 주파수 대역별로 대비를 재가중 (미세 텍스처 대 거친 대비)
- **필믹 톤 매핑 (Filmic Tone Map)** — 피벗 대비와 채도 복원을 갖춘 순수 Reinhard / Hable 하이라이트 롤오프, 고대비 단일 노출용
- **벨비아 (Velvia)** — 휘도 가중 채도 부스트로 그림자를 보존하면서 바랜 색을 강렬하게 살림
- **필름 네거티브 (Film Negative)** — 스캔한 컬러 네거티브를 반전하고, 주황색 필름 베이스를 나눠 제거하며 출력 감마 적용
- **디프린지 (Defringe)** — 고대비 가장자리의 보라 / 초록 색수차 프린지를 탈색
- **엠보스 (Emboss)** — 휘도 높이 필드에서 방향광 부조 생성
...
사각형 / 올가미 / 마법봉 / 빠른 선택, **교체 / 추가 / 빼기 / 교차** 모드. **퀵 마스크 모드** (`Q`) — 마스크를 그려서 작업하는 워크플로우용. **선택 영역 획**(Stroke Selection) 다이얼로그.

### 애니메이션 및 만화

- **애니메이션** — 프레임 타임라인 도크: **+ Frame** 버튼이 평면화된 그림을 스냅샷하고, 선택한 FPS로 재생하며, 어니언 스킨이 이전 프레임을 보여 줍니다. 프레임은 미리보기용이며 애니메이션 내보내기는 없습니다
...
| 클러스터 | 도크 |
|---|---|
| 드로잉 | Color · Brush · Bucket · Swatches |
...
- **Export image…** 명령은 선택한 파일 형식에 따라 PNG, JPEG, WebP, TIFF 또는 BMP로 기록합니다(투명도가 없는 JPEG와 BMP는 흰색 배경 위에 기록). 만화 프로젝트는 페이지를 **CBZ** 또는 **PDF**로 내보냅니다. 탭을 저장한 것으로 치는 것은 **Save as PSD…** 명령뿐이므로, 내보낸 뒤에도 닫을 때 저장되지 않은 변경 사항을 확인합니다
...
| Delete | `.xmp` / 주석 sidecar와 함께 휴지통으로 이동 (실행 취소 가능); 휴지통이 없는 드라이브(메모리 카드, USB 메모리, 네트워크 드라이브)에서는 영구 삭제를 확인할 때까지 파일을 남김 |
...
| `film_negative_image` / `graduated_density_image` | 스캔한 네거티브 반전; 선형 그라데이션 농도 그라데이션 적용 |
| `filmic_tonemap_image` / `tone_equalizer_image` / `detail_equalizer_image` | 필믹 하이라이트 롤오프; 영역별 노출; 대역별 대비 |
| `colormap_image` / `false_color_image` | 휘도를 viridis/magma/jet 컬러맵으로 재착색; 폴스 컬러 노출 스케일 |
...
| `levels_image` / `curve_image` | 흑/백점 + 감마 레벨; 톤 커브(S 커브 / 그림자 올리기 / 하이라이트 압축) |
| `auto_color_balance_image` / `channel_mixer_image` | 자동 화이트 밸런스(4가지 방법); 3×3 채널 믹서 + 흑백 변환 |
...
├── cli.py                   # 헤드리스 배치 CLI(Qt 불필요)
├── Imervue_main_window.py   # 메인 윈도우(QMainWindow) — 5개 탭 마운트
├── gpu_image_view/          # IMERVUE 탭 — GPU 뷰어, 딥 줌, 타일 월
│   ├── actions/             #   뷰어 동작(삭제 / 선택 / 비교 / 슬라이드쇼)
│   └── images/              #   로딩 계층(디코드 워커, 폴더 스캔, 프리페치)
├── gui/                     # Qt 셸 — 대화상자, 사이드 패널, 현상 패널, 캔버스
├── paint/                   # PAINT 탭 — 본격 래스터 편집기
│   ├── docks/               #   도크 패널(색상 / 브러시 / 레이어 / 머티리얼 / …)
│   └── tools/               #   포인터 툴 핸들러
├── puppet/                  # PUPPET 탭 — 2D 리깅 퍼펫 애니메이터 + Cubism 상호운용
├── desktop_pet/             # 데스크톱 펫 탭 — 프레임리스 오버레이 + 라이브 드라이버 및 연동
├── image/                   # 순수 이미지 코어(Qt 비의존) — recipe 파이프라인, 필터, 코덱,
│                            #   피라미드 / 타일 매니저, XMP, pHash, 메타데이터
├── library/                 # SQLite 라이브러리 인덱스, 스마트 앨범, CLIP 검색, 컬링
├── export/                  # 내보내기 생성기(컨택트 시트, 웹 갤러리, MP4, 치트시트)
├── macros/                  # 매크로 기록 / 재생
├── menu/                    # 메뉴 정의(file / tools / filter / 우클릭 / …)
├── mcp_server/              # Model Context Protocol stdio 서버
├── multi_language/          # i18n(en / zh-tw / zh-cn / ja / ko) + 검증
├── external/                # 외부 편집기 연동
├── plugin/                  # 플러그인 시스템(base / manager / downloader / pip installer)
├── sessions/                # 세션 및 워크스페이스 직렬화 + 마이그레이션
├── system/                  # OS 연동 — 테마, UI 스케일, 파일 연결,
│                            #   클립보드 모니터, 트리 감시, 배치 삭제, 로깅
└── user_settings/           # 영속 다중 프로필 설정, 태그, 별점, 북마크
```

트리 전반에 두 가지 규칙이 적용됩니다:

- **순수 로직과 Qt를 분리한다.** 단일 이미지 도구의 표준 형태는 `image/<feature>.py`(NumPy / Pillow, 워커 스레드나 디스플레이 없는 테스트에서 임포트 가능) + `gui/<feature>_dialog.py`(Qt 셸) + `menu/extra_tools_menu.py`의 항목 하나입니다.
- **큰 Qt 클래스는 위임한다.** `GPUImageView`, `PetWindow`, `PaintWorkspace`는 이벤트 오버라이드와 수명 주기만 유지하고, 동작은 이름 있는 협력자(`InputController`, `OverlayPainter`, `PetInteraction`, `ToolDispatcher` …)에 둡니다. 그 수학은 다시 순수 모듈로 추출되어 GL 컨텍스트 없이 단위 테스트됩니다.

### 렌더링 파이프라인 (Imervue 탭)

1. `GPUImageView`는 `QOpenGLWidget`을 확장
2. 두 개의 GLSL 1.20 프로그램 (textured quads + 단색 사각형)
3. LRU 텍스처 캐시 — 256 타일 소프트 상한(하드 상한 512), VRAM 예산은 GL 드라이버에서 탐지하고 1.5 GB로 폴백하며 사용자가 재정의 가능
...
- **키**: `{path}|{mtime_ns}|{file_size}|{thumbnail_size}|{recipe_hash}`의 MD5 — recipe 해시 덕분에 현상 편집이 썸네일에 반영되면서도 다른 모든 항목을 무효화하지 않습니다
