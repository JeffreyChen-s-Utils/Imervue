...
- **일괄 작업** — 이름 변경, 이동/복사, 선택한 이미지 회전. 이동·복사는 같은 이름의 파일을 덮어쓰지 않고 `name_1`로 둡니다. Imervue에서 이름을 바꾸거나 이동한 사진(일괄 이름 변경, 토큰 일괄 이름 변경, 폴더 트리, 이동 / 복사, 듀얼 창, 스테이징 트레이, 이미지 정리)은 별점·즐겨찾기·태그·컬러 라벨·제목·메모·선별 표시를 유지하며, `.xmp`와 주석 sidecar도 함께 옮겨집니다
