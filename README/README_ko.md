...
탭 5 — **Desktop Pet**은 모든 `.puppet` 리그를 데스크톱 위의 프레임리스 / 투명 / 항상 위 오버레이로 실행합니다. 앱 내 탭은 컨트롤 패널이며, 실제 캐릭터는 Puppet 런타임을 공유하는 별도의 최상위 창에 거주합니다 — 동일한 `PuppetCanvas`, 동일한 파라미터 / 모션 / 물리 파이프라인, 동일한 라이브 입력 드라이버.

### 윈도우 동작

| 기능 | 설명 |
|---|---|
| 프레임리스 오버레이 | 창 크롬 없음, 작업 표시줄 항목 없음; `Qt.WindowStaysOnTopHint`를 통해 다른 모든 창 위에 위치합니다. |
| 투명 배경 | `WA_TranslucentBackground` + 알파 인식 GL 서피스 포맷 + `glClearColor(0,0,0,0)` — 퍼펫이 그리지 않는 모든 픽셀은 데스크톱을 그대로 비춥니다. |
| 드래그로 이동 | 캐릭터에서 왼쪽 드래그로 위치 재조정. 화면 가장자리로부터 설정 가능한 스냅 임계값(기본 24 px) 안에서 놓으면 가장자리에 딱 맞게 **스냅**됩니다. 빠른 드래그로 오버슈팅된 경우 화면 안으로 다시 클램프되어 펫이 화면 밖으로 사라지지 않습니다. |
| 클릭 통과 토글 | 선택적 `Qt.WindowTransparentForInput` 모드 — 모든 클릭이 펫 뒤의 데스크톱 / 앱으로 통과합니다. |
| 앵커 잠금 | 원클릭으로 펫의 위치를 고정하여 우발적인 드래그가 펫을 움직이지 못하게 합니다. |
| 항상 아래 | `WindowStaysOnTopHint`에서 `WindowStaysOnBottomHint`로 전환하여 펫을 데스크톱 위젯처럼 모든 창 뒤에 배치합니다 (`WindowDoesNotAcceptFocus`와 조합). |
| 전체 화면 시 숨김 | 1 Hz 폴러가 Win32 `GetWindowRect` API(Windows)를 통해 활성 창을 감시하고, 다른 앱이 펫의 모니터에서 전체 화면을 점유하는 동안 펫을 자동으로 숨깁니다. |
| 숨겨졌을 때 일시 정지 | 오버레이가 숨겨진 동안 33 ms 페인트 틱이 멈춰 휴면 펫은 CPU를 전혀 사용하지 않습니다. `showEvent`에서 재개합니다. |
| 크기 프리셋 | 소형 (200×300) / 중형 (320×480) / 대형 (480×720); 중앙 앵커 방식이므로 크기 조정 시 펫이 화면을 가로질러 튀지 않습니다. |
| 불투명도 슬라이더 | `setWindowOpacity`를 통한 윈도우 레벨 불투명도 0.1 – 1.0 — `WA_TranslucentBackground` 컴포지트와 창별 알파가 결합되어 퍼펫 픽셀만 어둡게 하는 것이 아닌 매끄러운 페이드를 제공합니다. |
| 위치 영속성 | 드래그를 놓을 때마다 스냅 후의 `(x, y)`가 `user_setting_dict["desktop_pet"]["position"]`에 기록됩니다. 다음 실행 시 펫은 그 화면 위치로 돌아옵니다; 다중 모니터 연결이 끊긴 경우 주 화면의 우측 하단 모서리로 폴백됩니다. |

### 상호작용

| 기능 | 설명 |
|---|---|
| **본체에서 왼쪽 클릭** | 역 팬 / 줌 행렬을 통해 클릭을 퍼펫 캔버스 좌표로 매핑하고, 문서의 `HitArea` 항목에 대해 기존 `hit_test()`를 실행하여, 해당 hit drawable을 덮는 모션이 있으면 연결된 모션을 재생합니다. 일치하는 것이 없으면 말풍선에서 라운드 로빈 인사말로 폴백합니다. |
| **어디서나 오른쪽 클릭** | 다음 항목이 포함된 컨텍스트 메뉴를 엽니다: 펫 숨기기, **Live drivers** 서브메뉴 (6개의 체크 가능한 토글), **Play motion** 서브메뉴 (`document.motions`로부터 채워짐), **Apply expression** 서브메뉴 (`document.expressions`로부터 채워짐), 위치 잠금, 클릭 통과, 항상 아래, 전체 화면 시 숨김, 말풍선 토글, 그리고 **Size** 서브메뉴. |
| **말풍선** | 둥근 본체 + 꼬리가 있는 프레임리스 / 투명 / 항상 위 QWidget. 클릭 시 펫 위에 팝업되어 약 4초간 유지된 뒤 400 ms에 걸쳐 페이드 아웃합니다. 펫의 geometry에 앵커되어 펫을 드래그하면 말풍선이 함께 따라옵니다. 숨기기 / 리그 교체 시 자동 해제. |
| **시스템 트레이** | Show / Hide (체크 가능), Click-through, Open puppet…, Hide pet. 왼쪽 클릭으로 가시성 토글; 오른쪽 클릭으로 메뉴를 엽니다. `sync_visibility` / `sync_click_through`를 통해 워크스페이스의 체크 상태를 미러링합니다. |

### 라이브 드라이버 (지연 초기화)

각 드라이버는 처음 활성화될 때 인스턴스화되므로 휴면 펫은 타이머 / 스레드 비용이 전혀 들지 않습니다:

- **Auto idle** — `IdleDriver`를 통해 표준 파라미터 (`ParamBreath` 등)에 호흡 + 드리프트를 적용.
- **Idle motions** — `IdleMotionCycler` + 번들된 `MotionPlayer`를 통해 `Idle` 그룹 모션을 무작위 순환.
- **Auto-blink** — `InputEngine.set_blink_enabled`를 통해 약 4.5초마다 `ParamEyeLOpen` / `ParamEyeROpen`에 코사인 close-open 사이클.
- **Drag-track head** — `InputEngine.set_drag_enabled`를 통해 커서 오프셋을 `ParamAngleX/Y` + `ParamEyeBallX/Y`에 매핑.
- **Mic lip-sync** — `InputEngine.set_lipsync_enabled`를 통해 마이크 RMS를 `ParamMouthOpenY`에 매핑 (`sounddevice` 필요).
- **Webcam tracking** — `WebcamTracker`를 통해 MediaPipe FaceLandmarker → 머리 + 눈 + 입 (`opencv-python` + `mediapipe` 필요).

### 영속성

`Imervue/desktop_pet/settings.py`는 `user_setting_dict["desktop_pet"]` 위에 다음을 계층화합니다:

- 로드 시 모든 키에 대한 기본값 + 범위 클램핑으로, 손상된 설정 파일이 실행을 무너뜨릴 수 없도록 함.
- 한 단계 깊이 병합 — 새로운 키가 누락된 이전 설정 파일도 여전히 완전한 상태 dict를 생성.
- `drivers` 하위 dict에 대한 전방 호환 — 알 수 없는 드라이버 키는 그대로 라운드 트립되므로, 새로운 드라이버를 추가하는 미래 버전이 기존 파일을 깔끔하게 읽을 수 있음.

사용자가 조정 가능한 모든 표면 (위치, 크기, 불투명도, 클릭 통과, 앵커, 항상 아래, 전체 화면 시 숨김, 말풍선, 스냅 임계값, 각 드라이버, 마지막 로드된 리그, 시작 시 표시)은 이 헬퍼를 통해 라운드 트립되므로 펫은 다음 실행 시 동일한 상태로 돌아옵니다.

### 구현

| 파일 | 역할 |
|---|---|
| `Imervue/desktop_pet/pet_window.py` | 최상위 오버레이 — 프레임리스 / 항상 위 / `WA_TranslucentBackground`. `PuppetCanvas(pet_mode=True)`를 호스팅하고, 드래그 이동, hit-detect, 컨텍스트 메뉴, 말풍선 통합, 전체 화면 감지기 배선, 드라이버, 영속성 write-through를 소유. |
| `Imervue/desktop_pet/edge_snap.py` | 순수 Python 스냅 수학 (Qt 없음) — 단위 테스트 가능한 모서리 / 가장자리 도킹 + 오버슈트 클램프. |
| `Imervue/desktop_pet/settings.py` | 영속성 헬퍼 — load / save / update / clamp. |
| `Imervue/desktop_pet/speech_bubble.py` | 사각형 앵커 위치 지정 + 페이드 애니메이션이 있는 프레임리스 둥근 말풍선 오버레이. |
| `Imervue/desktop_pet/fullscreen_detector.py` | 1 Hz 폴 루프로 전경 창의 사각형을 읽고 (Windows에서 Win32 ctypes; 다른 곳에서는 no-op 폴백) `state_changed(bool)`을 emit. |
| `Imervue/desktop_pet/pet_workspace.py` | 컨트롤 패널 탭. 오버레이를 지연 생성하고, 모든 토글 / 슬라이더 / 콤보를 체크박스 또는 스핀박스로 노출하며, 마지막 로드된 리그 + 시작 시 표시 동작을 영속화. |
| `Imervue/desktop_pet/tray_icon.py` | 시스템 트레이 헬퍼 — 세션당 단일 인스턴스, 워크스페이스 체크 상태와 동기화. |

`PuppetCanvas.__init__(pet_mode=True)`는 편집기의 투명도 체커 배경과 선택 오버레이를 단락 처리합니다; 렌더 경로의 나머지(메시 VBO, 모션 플레이어, 물리, 표정, 포즈 그룹)는 Puppet 탭과 동일합니다.

모든 UI 문자열은 다섯 개의 기본 언어 팩(English, 繁體中文, 简体中文, 日本語, 한국어)에 정의된 키를 통해 `language_wrapper.language_word_dict.get(...)`로 라우팅됩니다.
