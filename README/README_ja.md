...
- **アニメーション再生** — GIF / APNG、再生 / 一時停止 / フレーム送り / 速度調整付き。デコード後に 512 MB を超えるアニメーションは、最初に全フレームをデコードせず再生しながら 1 フレームずつデコード。10 ms 以下のフレームはブラウザーと同じく 100 ms 表示。複数ページの TIFF(スキャンした文書)は再生せず 1 ページずつ表示し、`,` と `.` でめくります(表示は「2/5 ページ」)。カメラが JPEG に埋め込むプレビュー(MPF)は 2 枚目のフレームとして表示しません
