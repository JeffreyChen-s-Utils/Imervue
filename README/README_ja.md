...
- **XMP サイドカー**(`.xmp` コンパニオン)— レーティング / タイトル / 説明 / キーワード / カラーラベルを other XMP-aware photo managers と双方向同期(`defusedxml` で安全に解析)。保存時は既存の sidecar にマージします。変わるのはこれらの項目だけなので、RAW 現像ソフトが保存した現像設定・切り抜き・履歴は残り、解析できない sidecar は上書きしません。
