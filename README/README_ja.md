...
- **EXIF エディタ** ダイアログ — 説明・作者・著作権・カメラ・コメント（Unicode 可）を追加パッケージなしで JPEG に書き込み、画素と他のタグはそのまま（WebP は `piexif` が必要）
...
- **GPS ジオタグエディタ** — EXIF GPS の緯度経度を読み書き。JPEG は追加パッケージ不要で、画素・他のタグ・サムネイルはそのまま（他の形式は `piexif` が必要）
