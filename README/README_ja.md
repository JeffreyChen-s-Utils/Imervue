...
- **他のプログラムで変更されたファイル** — 他のプログラムが画像を上書き保存すると(外部エディタによる直接の上書きでも、コピーを書いてからの置き換えでも)、ビューアは新しい版を表示します。ディープズームで開いている画像は最後の書き込みから 0.5 秒後(Imervue が前面に戻ったときにも再確認)、グリッドのサムネイルは数秒以内に更新されます
