...
- **バッチ操作** — 選択画像のリネーム、移動 / コピー、回転。移動・コピーは同名ファイルを上書きせず `name_1` として置きます。Imervue でリネーム・移動した写真(バッチリネーム、トークンバッチリネーム、フォルダツリー、移動 / コピー、デュアルペイン、ステージングトレイ、画像整理)は評価・お気に入り・タグ・カラーラベル・タイトル・メモ・選別フラグを保ち、`.xmp` と注釈の sidecar も一緒に移ります。フォルダを Imervue で開いている間に他のプログラムでリネームした写真も同様です
