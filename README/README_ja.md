...
- **ディープズームピラミッド** — 512×512 タイルのマルチレベル LANCZOS リサンプリング。タイル LRU は 256 エントリ保持(ハード上限 512)。VRAM 予算は起動時に GL ドライバから検出し、取得できない場合は 1.5 GB にフォールバック。`vram_limit_mb` 設定で上書き可能(クランプされ、黙って無視されることはありません)。最大 8× の異方性フィルタリング。Pillow の安全上限（1 億 7900 万画素）を大きく超えるパノラマも開ける（上限はメモリ量に合わせて決まり、16 GB なら約 13 億画素）
...
- **リスト(詳細)** — `Ctrl+L` で切り替え。列: プレビュー · ラベル · 評価 · 名前 · 解像度 · サイズ · タイプ · 更新日時。`Delete` で選択した行を削除し、`Ctrl+Z` で元に戻せます。評価(`0`–`5`)、カリング(`P` / `Shift+X` / `U`)、カラー(`F1`–`F5`)のキーも選択した行に付きます(グリッドと同じ)
