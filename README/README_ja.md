...
- **ディープズームピラミッド** — 512×512 タイルのマルチレベル LANCZOS リサンプリング。タイル LRU は 256 エントリ保持(ハード上限 512)。VRAM 予算は起動時に GL ドライバから検出し、取得できない場合は 1.5 GB にフォールバック。`vram_limit_mb` 設定で上書き可能(クランプされ、黙って無視されることはありません)。最大 8× の異方性フィルタリング。Pillow の安全上限（1 億 7900 万画素）を大きく超えるパノラマも開ける（上限はメモリ量に合わせて決まり、16 GB なら約 14 億画素）
...
- **カラーマネジメント** — カラープロファイルを埋め込んだ写真(スマートフォンの Display P3、カメラの Adobe RGB、CMYK、Photoshop がグレースケール画像に埋め込む Dot Gain 20% や Gray Gamma 1.8 などのグレープロファイル)は、ビューアとサムネイルで sRGB に変換して表示。プロファイルなし・sRGB の画像はそのまま表示します
- **途中で切れたファイル** — 途中までしかない JPEG・PNG・TIFF・GIF・BMP(ダウンロードやコピーの中断、故障したメモリーカードから復旧した写真)も、開けないのではなく、ブラウザと同じく読めた部分までを表示します
- **他のプログラムで変更されたファイル** — 他のプログラムが画像を上書き保存すると(外部エディタによる直接の上書きでも、コピーを書いてからの置き換えでも)、ビューアは新しい版を表示します。ディープズームで開いている画像は最後の書き込みから 1 秒以内、グリッドのサムネイルとリストの行は数秒以内に更新されます
- **16 ビット・浮動小数点のグレースケール** — 16 ビットのグレースケール PNG / TIFF(スキャン、深度マップ、科学・天体の画像)や浮動小数点 TIFF も、ほぼ真っ白や真っ黒ではなく、ビューア・サムネイル・プレビュー・ツールで本来の明るさで表示されます
- **隠しファイル** — Windows で隠し属性の付いたファイル(エクスプローラーやフォルダーツリーでも非表示)と、ドットで始まる名前(macOS がメモリーカードやネットワークドライブで写真ごとに書く `._photo.jpg` など)は、サムネイル一覧、フォルダーアイコン、バッチツールのフォルダー一覧、監視フォルダー、ライブラリのスキャン、CLI、MCP サーバーのフォルダーツールから除外されます。再帰スキャンは `$RECYCLE.BIN` や Mac の `.Trashes` などの隠しフォルダーに入りません。意図して開いた隠し画像はそのまま開けます
- **どの名前の JPEG も** — `.jpe`・`.jfif`・`.jif` も `.jpg` と同じく開けます(Windows の Chrome や Edge はダウンロードした写真をよく `.jfif` で保存します)。ビューア、JPG フィルター、バッチツール、CLI のどれでも同じです
- **その他の形式** — ICO、TGA、DDS、QOI、JPEG 2000(.jp2／.j2k／.jpf／.jpx)、Netpbm(PPM／PGM／PBM／PNM)、PCX、PSD(統合画像) は Pillow 自身が読み込み、表示できます。その場での回転などの書き戻しは行わないため、編集は「名前を付けて保存」/「エクスポート」で保存します
- **アニメーション再生** — GIF / APNG、再生 / 一時停止 / フレーム送り / 速度調整付き。デコード後に 512 MB を超えるアニメーションは、最初に全フレームをデコードせず再生しながら 1 フレームずつデコード。10 ms 以下のフレームはブラウザーと同じく 100 ms 表示。複数ページの TIFF(スキャンした文書)は再生せず 1 ページずつ表示し、`,` と `.` でめくります(表示は「2/5 ページ」)。カメラが JPEG に埋め込むプレビュー(MPF)は 2 枚目のフレームとして、APNG の既定画像(APNG 非対応のプログラム向けの静止画)は 1 枚目のフレームとして表示しません

### 閲覧モード

- **グリッド**(デフォルト)— 仮想化タイルグリッド、ホバープレビュー(500 ms 遅延)
- **リスト(詳細)** — `Ctrl+L` で切り替え。列: プレビュー · ラベル · 評価 · 名前 · 解像度 · サイズ · タイプ · 更新日時。`Delete` で選択した行を削除し、`Ctrl+Z` で元に戻せます。評価(`1`–`5`)、お気に入り(`0`)、カリング(`P` / `Shift+X` / `U`)、カラー(`F1`–`F5`)のキーも選択した行に付きます(グリッドと同じ)
...
- ピクセルビュー(`Shift+P`)— 400% 以上のズームでピクセルごとの RGB / HEX を表示し、画面上のピクセルが 40,000 以下になるとピクセルグリッドも表示
...
- **ブックマーク** — 最大 5000 パス
- **レーティング** — 0-5 つ星(`1`-`5`)+ お気に入りハート(`0`)。グリッドでは選択中のサムネイル、なければ矢印キーで選んだもの、なければマウスの下のものに適用
- **カラーラベル** — フラグ式の赤 / 黄 / 緑 / 青 / 紫(`F1`-`F5`)
- **カリング**(Culling)— 3 状態フラグ(`P` = キープ、`Shift+X` = 拒否、`U` = フラグ解除)。状態でフィルタ、拒否済みの一括削除。自動カリングは近似重複グループごとに最もシャープなフレームをキープし、残りを拒否
- **階層タグ** — `animal/cat/british` のようなツリーパス。子孫が自動的にマッチ。右クリックの **一括操作** > **キーワードを索引**(サムネイル選択時)は Lightroom／darktable のキーワード階層(`Places|Taiwan|Taipei`)を親の下に登録します
...
- 名前（エクスプローラーと同じ自然順: `img2` が `img10` より前）/ 更新日 / 作成日 / 撮影日時(カメラの EXIF 時刻、なければ更新日) / サイズ / 解像度でソート(昇順 / 降順)
...
- **ライブラリ検索** — SQLite マルチルートインデックスを、ファイル名・最小幅 / 高さ・ファイルサイズで検索(最大 2000 件。ダブルクリックで開きます)
...
- **XMP サイドカー**(`.xmp` コンパニオン)— レーティング / タイトル / 説明 / キーワード / カラーラベルを他の XMP 対応写真管理ソフトと双方向同期(`defusedxml` で安全に解析)。保存時は既存の sidecar にマージします。変わるのはこれらの項目だけなので、RAW 現像ソフトが保存した現像設定・切り抜き・履歴は残り、解析できない sidecar は上書きしません。`photo.xmp`（Lightroom、Bridge）のほか、darktable と digiKam が書く `photo.jpg.xmp` も、それが唯一の sidecar なら読み込んで更新します。カラーラベルは Lightroom の表記（`Red` … `Purple`）と Bridge の表記（`Select`、`Second`、`Approved`、`Review`、`To Do`）を理解し、エクスポート時は Lightroom の表記で書き込みます。除外された写真(Lightroom、Bridge、darktable の `xmp:Rating` -1)は選別の「除外」になり、「除外」は -1 としてエクスポートします。サイドカーのないファイルは、ファイル自体に埋め込まれた XMP と EXIF レーティング(JPEG、PNG、WebP、TIFF、CR3、RW2、ORF、RAF)を読み込み・取り込みます。Lightroom は JPEG の評価とキーワードを、Windows エクスプローラーは星をこの形で保存します。
...
- **スプリットトーニング** — シャドウ / ハイライト色相 + 彩度、バランスピボット付き
...
- **フレームとキャプション(Frame & Caption)** — 任意の色のマット枠、オプションのポラロイド風下部帯、個別に色を指定できるキャプション
...
- **クロップ / 水平補正** — 正規化されたクロップ矩形 + 最大 ±15° の水平補正で、最大内接矩形に自動クロップ
...
- **エクスポートプリセット** — バッチエクスポートで選択: Web 1600 px / 4K Web 3840 px / Print 300 DPI PNG / Instagram 1080 × 1080 正方形 / Thumbnail 400 px、またはカスタム
- **透かし** — バッチエクスポートで、テキストの透かしを四隅のいずれかまたは中央に、不透明度を指定して配置。エクスポートしたコピーにのみ適用
- **Save As / Export** — PNG / JPEG / WebP / BMP / TIFF（Pillow が AVIF に対応していれば AVIF、`pillow-heif` があれば HEIC、`pillow-jxl-plugin` があれば JPEG XL も）。ロッシー形式には品質スライダー。カメラ・レンズ・撮影日時の EXIF を引き継ぎ、位置情報は任意（**メタデータ**：すべて／位置情報以外／なし）。提案されるファイル名は未使用の名前（`photo.png` の隣なら `photo_1.png`）で、既存のファイル（特に元の写真そのもの）は確認してから置き換え
...
- **Web ギャラリー HTML** — `index.html` + JPEG サムネイル + インラインライトボックスを含む自己完結型フォルダ。**クライアントレビュー** をオンにすると各画像の下にコメント欄が付き、コメントはレビュー担当者のブラウザーに残り、まとめて 1 つの JSON ファイルとしてダウンロードできます
...
**File > External Editors…** でプログラム(画像エディタなど)を登録し、**File > Open in External Editor** から起動します。エディタで保存すると、ビューアは自動で新しい版を表示します。
...
暗室トーニングの 3 兄弟 — **覆い焼き(Dodge)**(明るく)、**焼き込み(Burn)**(暗く)、**スポンジ(Sponge)**(彩度を下げる)— は、ブラシで重み付けした局所的な調整を描き込みます。覆い焼きと焼き込みは中間調に作用します。3 つともオプションはありません。

シングルキーショートカット: `B / E / G / I / M / L / W / V / T / U / R / P / S / C / Z / H`。`Shift+R/E/I/P` で図形バリアントを切り替え。

### ブラシ

6 種類のブラシ — 鉛筆 / ペン / マーカー / エアブラシ / 水彩 / 墨 — と、それらをベースにしたプリセット(Crayon、Highlight、Sumi calligraphy …)。ブラシドックでサイズ / 不透明度 / 硬さ / 密度 / ブレンドモードを設定し、オプションバーにはサイズ / 不透明度 / 硬さがあります。タブレットペンの筆圧は **Settings > Pressure Curve…** で設定したカーブに従ってサイズと不透明度を変化させ、マウスでは常に最大筆圧で描画されます。選択範囲からブラシ先端をキャプチャ、**File > Import brush preset…**。

### レイヤー

サムネイル、表示切り替え、↑ / ↓ の並び替えボタン(または `Ctrl+[` / `Ctrl+]`)、ブレンドモード、不透明度、検索、ベクターレイヤー、1-bit レイヤーを備えたフルレイヤーパネル。**レイヤーマスク**(追加 / 選択から / 反転 / 適用)、**クリッピングマスク**、**レイヤー効果**(ドロップシャドウ / 外側の光彩 / 境界線)。色によるレイヤー分割、グラデーションマップのプリセット。

### 選択

矩形 / 投げ縄 / マジックワンド / クイック選択。**置換 / 追加 / 減算 / 交差** モード付き。**クイックマスクモード**(`Q`)。**選択範囲のストローク** ダイアログ。

### アニメーション & 漫画

- **アニメーション** — フレームタイムラインドック: **+ Frame** で統合した絵をスナップショット、選んだ FPS で再生、オニオンスキンで前のフレームを表示。フレームはプレビュー用で、アニメーションのエクスポートはありません
...
各ドックは移動 / フローティング可能で、**Window** メニューから個別に開閉できます。**Settings > Workspace Layouts…** には組み込みの Default / Drawing / Comic / Compact レイアウトがあります。**Save current…** はレイヤー / カラー / ブラシ / ナビゲーター / 履歴 / リファレンスの各ドックのどれを表示しているかを名前付きで保存し、レイアウトを適用するとそれらのドックが表示 / 非表示になります。ツールオプションとドックのサイズは保存されません。

### ファイル I/O

- **Open PSD…** はファイルを 1 枚のレイヤーに統合して新しいタブで開きます。**Save as PSD…** はレイヤーをブレンドモード付きで書き出します(マスクとレイヤー効果は含みません)
- **Export image…** は選んだファイル形式に応じて PNG・JPEG・WebP・TIFF・BMP で書き出し(透明度を持たない JPEG と BMP は白背景に合成)。漫画プロジェクトはページを **CBZ** または **PDF** にエクスポート。タブの保存として扱われるのは **Save as PSD…** だけで、エクスポートした後も閉じるときには未保存の変更について確認します
- **自動保存** — アクティブタブに未保存の編集がある間、2 分ごとにスナップショットを作成。次回起動時にトーストでスナップショットを案内し、**File > Restore Autosave** で最新のものをアクティブタブに読み込みます。ステータスバーには最後のスナップショットを作成した時刻が表示されます。Imervue を閉じるときは、未保存の変更がある Paint タブについて確認します。
...
**Puppet** タブはゼロから構築された 2D リギングパペットアニメーションシステムです。メッシュ変形リギング、パラメーター、モーション、物理、表情、ポーズ、リップシンク、ウェブカメラ顔追跡を、**プロプライエタリな SDK を一切使わず**、**`live2d-py` も使わず**、`Imervue/puppet/FORMAT.md` で完全に文書化された完全オープンな `.puppet` ファイルフォーマットで実現します。
...
- **回転デフォーマー追加**(アンカー + 角度)/ **ワープデフォーマー追加**(rows × cols 双線形格子)を **Edit** メニューから
...
- **ポーズグループ** — 排他的な drawable 表示(武器の切り替え、口形バリアント)。**Pose** ドックで各グループに表示するメンバーを選択
...
- ドラッグ追従ヘッド — カーソルがキャンバス上を動くと、頭と目がカーソルの方を向く
- 自動まばたき。cosine open → close → open カーブ
- マイクリップシンク `sounddevice` RMS → `ParamMouthOpenY`(オプション依存)
- ウェブカメラ顔追跡 OpenCV + MediaPipe Tasks の FaceLandmarker → 頭部 yaw / pitch / roll + 目 / 口の開閉(オプション依存)
...
- **画面キャプチャ…** キャラクターだけを rig 本来のサイズ(長辺は最大 4096 px)・透明背景の PNG として保存
- **録画…** 30 FPS フレームループを切り替え、`imageio` で GIF / WebM / MP4 として書き出し。キャラクターは白背景の 1080 px に収めて描画(これらのフレームはアルファを持ちません)
...
| 非表示中は一時停止 | 非表示の間は再描画を停止します。ライブドライバーのタイマーは動き続けます。 |
...
- **どこでも右クリック** — コンテキストメニューを開きます:ペットを非表示、ライブドライバー、モーション再生(rig 内のすべてのモーション一覧)、表情適用、ポーズ(各ポーズグループで表示するメンバーを選択)、位置のロック、クリックスルー、常に最背面、フルスクリーン時に非表示、スピーチバブル、サイズ。
- **システムトレイアイコン** — 左クリックで表示切り替え、右クリックで表示 / 非表示、クリックスルー、Open puppet、Hide pet。

### ライブドライバー

タブまたは右クリックメニューから好きな組み合わせを選択。オートアイドル、アイドルモーション、オートまばたきはデフォルトでオン、残りはオフ — 必要なものだけ有効にしてください。

- **オートアイドル** — 呼吸 + 微妙なドリフトでキャラクターが生きているように感じられます。
- **アイドルモーション** — rig の Idle グループのモーションをランダムに循環再生。
- **オートまばたき** — 数秒ごとに自然なまばたきサイクル。
- **ドラッグ追従ヘッド** — カーソルがペットの上にある間、頭と目がカーソルの方を向きます。
- **マウス視線** — 画面上のどこにあっても、目と頭がカーソルを追いかけます。
...
- **`time_of_day_greetings`** — ローカル時刻の時間帯ごとの挨拶(`morning` 5–11 時、`afternoon` 12–17 時、`evening` 18–21 時、`night` 22–4 時)。`greetings` より先に使われ、セリフのない時間帯は `greetings` にフォールバックします。
- **`hit_responses`** — `HitArea` ごとのセリフ。キーは rig で定義されたヒットエリアの ID と一致する必要があります。
- **`motion_lines`** — モーションごとのセリフ。ヒットエリアのクリックでその名前のモーションが再生されたときに話します(コンテキストメニューから開始したモーションでは話しません)。
...
| ショートカット | 動作 |
|----------|--------|
| 矢印キー | グリッド: フォーカス枠を移動(Enter で開く) / ディープズーム: 左 / 右で画像切り替え |
| Ctrl+Shift+←/→ | 画像を含む前 / 次のサイブリングフォルダへジャンプ |
| Alt+← / Alt+→ | 履歴を戻る / 進む |
| Ctrl+G | N 番目の画像へジャンプ |
| X | ランダム画像へジャンプ |
| Home | 画像をウィンドウに合わせる(グリッドでは先頭までスクロールして戻る) |
...
| Shift+F | ウィンドウに合わせる |
| - / = | 縮小 / 拡大 |
| V | 読書モード(幅に合わせ、スクロールして読み、末尾で次の画像へ進む) |
| L | ルーペ: カーソルに追従する拡大鏡(サムネイル上でも使える) |
| H | RGB ヒストグラム切り替え |
| F8 / Ctrl+F8 | OSD オーバーレイ / デバッグ HUD |
| Shift+P | ピクセルビュー切り替え(400% 以上で RGB を表示。画面上のピクセルが 40,000 以下になるとグリッドも表示) |
| Shift+M | カラーモード循環 |
| B | ブックマーク切り替え |
...
各セッションのログ `imervue.log` も同じフォルダに書き込まれます(そのフォルダが読み取り専用の場合は `%LOCALAPPDATA%\Imervue`、Windows 以外では `~/.cache/imervue`)。前回のセッションのログは `imervue.previous.log` として隣に残るため、クラッシュした後に Imervue を再び起動しても、その原因がわかるログが残っています。問題を報告するときは両方を添付してください。
