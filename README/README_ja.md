...
6 種類のブラシ — 鉛筆 / ペン / マーカー / エアブラシ / 水彩 / 墨 — と、それらをベースにしたプリセット(Crayon、Highlight、Sumi calligraphy …)。ブラシドックでサイズ / 不透明度 / 硬さ / 密度 / ブレンドモードを設定し、オプションバーにはサイズ / 不透明度 / 硬さがあります。タブレットペンの筆圧でサイズと不透明度が変わります。選択範囲からブラシ先端をキャプチャ、**File > Import brush preset…**。
