...
- **スライドショー MP4** — H.264 動画、FPS / 画像ごとの保持秒数 / フェードトランジション設定可(`imageio-ffmpeg`)
...
タブ 5 — **Desktop Pet** は任意の `.puppet` rig を、フレームレスかつ透明で常に最前面のデスクトップオーバーレイとして実行します。アプリ内のタブはコントロールパネルで、実際のキャラクターは Puppet ランタイムを共有する独立したトップレベルウィンドウに表示されます — 同じ `PuppetCanvas`、同じパラメーター / モーション / 物理パイプライン、同じライブ入力ドライバーです。

### ウィンドウの挙動

| 機能 | 説明 |
|---|---|
| フレームレスオーバーレイ | ウィンドウクロームなし、タスクバーエントリなし。`Qt.WindowStaysOnTopHint` により他のすべてのウィンドウの上に常駐。 |
| 透明な背景 | `WA_TranslucentBackground` + アルファ対応の GL サーフェスフォーマット + `glClearColor(0,0,0,0)` — パペットが描画しないすべてのピクセルからデスクトップが透けて見えます。 |
| ドラッグして移動 | キャラクターを左ドラッグして位置を変更。設定可能なスナップしきい値(デフォルト 24 px)内で画面端の近くで離すと、その端に**スナップ**して密着します。高速ドラッグのオーバーシュートは内側にクランプされ、ペットが画面外で迷子になりません。 |
| クリックスルー切り替え | オプションの `Qt.WindowTransparentForInput` モード — すべてのクリックがペットの背後のデスクトップ / アプリに通り抜けます。 |
| アンカーロック | ワンクリックでペットの位置を固定し、誤ドラッグで移動しないようにします。 |
| 常に最背面 | `WindowStaysOnTopHint` から `WindowStaysOnBottomHint` に切り替え、ペットをすべてのウィンドウの背後にデスクトップウィジェットとして配置(`WindowDoesNotAcceptFocus` と組み合わせ)。 |
| フルスクリーン時に非表示 | 1 Hz のポーラーが Win32 `GetWindowRect` API(Windows)経由でアクティブウィンドウを監視し、他のアプリがペットのモニターでフルスクリーンになっている間は自動的にペットを非表示にします。 |
| 非表示中は一時停止 | オーバーレイが非表示の間は 33 ms の描画ティックを停止するので、休止中のペットは CPU を一切消費しません。`showEvent` で再開します。 |
| サイズプリセット | Small(200×300)/ Medium(320×480)/ Large(480×720)。中心アンカーなので、リサイズ時にペットが画面を横切るように飛び跳ねません。 |
| 不透明度スライダー | `setWindowOpacity` によるウィンドウレベルの不透明度 0.1 – 1.0 — WA_TranslucentBackground コンポジット + ウィンドウごとのアルファによって、パペットピクセルを暗くするだけではなくスムーズなフェードが得られます。 |
| 位置の永続化 | ドラッグリリースごとのスナップ後の `(x, y)` が `user_setting_dict["desktop_pet"]["position"]` に書き込まれます。次回起動時にペットはその画面位置に戻ります。マルチモニター切断時はプライマリスクリーンの右下にフォールバックします。 |

### インタラクション

| 機能 | 説明 |
|---|---|
| **本体への左クリック** | 逆パン / ズーム行列を通じてクリックをパペットキャンバス座標にマップし、ドキュメントの `HitArea` エントリに対して既存の `hit_test()` を実行、ヒットした drawable をカバーするものがあればリンクされたモーションを再生。何もマッチしない場合は、スピーチバブルでラウンドロビン挨拶にフォールバック。 |
| **どこでも右クリック** | コンテキストメニューを開きます: ペットを非表示、**ライブドライバー** サブメニュー(6 個のチェック可能なトグル)、**モーション再生** サブメニュー(`document.motions` から自動生成)、**表情適用** サブメニュー(`document.expressions` から自動生成)、位置ロック、クリックスルー、常に最背面、フルスクリーン時に非表示、スピーチバブルの切り替え、**サイズ** サブメニュー。 |
| **スピーチバブル** | フレームレス / 透明 / 常に最前面の QWidget で、丸みのある本体 + テール付き。クリック時にペットの上にポップアップし、約 4 秒間保持された後、400 ms かけてフェードアウト。ペットのジオメトリにアンカーされるので、ペットをドラッグするとバブルも一緒に動きます。非表示 / rig 切り替え時に自動消去。 |
| **システムトレイ** | 表示 / 非表示(チェック可能)、クリックスルー、Open puppet…、Hide pet。左クリックで表示切り替え、右クリックでメニュー。`sync_visibility` / `sync_click_through` でワークスペースのチェック状態をミラーリング。 |

### ライブドライバー(遅延初期化)

各ドライバーは最初に有効化された時点でインスタンス化されるので、休止中のペットはタイマー / スレッドコストを一切支払いません:

- **オートアイドル** — `IdleDriver` 経由で標準パラメーター(`ParamBreath` 等)に呼吸 + ドリフト。
- **アイドルモーション** — `IdleMotionCycler` + 同梱の `MotionPlayer` で `Idle` グループのモーションをランダム循環。
- **オートまばたき** — `InputEngine.set_blink_enabled` 経由で `ParamEyeLOpen` / `ParamEyeROpen` に約 4.5 秒ごとの cosine 閉開サイクル。
- **ドラッグ追従ヘッド** — `InputEngine.set_drag_enabled` 経由でカーソルオフセット → `ParamAngleX/Y` + `ParamEyeBallX/Y`。
- **マイクリップシンク** — `InputEngine.set_lipsync_enabled` 経由でマイク RMS → `ParamMouthOpenY`(`sounddevice` が必要)。
- **ウェブカメラ追跡** — `WebcamTracker` 経由で MediaPipe FaceLandmarker → 頭部 + 目 + 口(`opencv-python` + `mediapipe` が必要)。

### 永続化

`Imervue/desktop_pet/settings.py` は `user_setting_dict["desktop_pet"]` の上に次のレイヤーを重ねます:

- すべてのキーのデフォルト + ロード時の範囲クランプにより、破損した設定ファイルでも起動がクラッシュしないように。
- 1 レベル深いマージ — 新しいキーが欠けている古い設定ファイルでも完全な状態 dict を生成。
- `drivers` サブ dict の前方互換性 — 未知のドライバーキーはそのままラウンドトリップされるので、新しいドライバーを追加する将来のバージョンが既存のファイルをクリーンに読み込めます。

ユーザーが調整可能なすべての項目(位置、サイズ、不透明度、クリックスルー、アンカー、最背面、フルスクリーン時非表示、スピーチ、スナップしきい値、各ドライバー、最後にロードした rig、起動時表示)はこのヘルパー経由でラウンドトリップするので、ペットは次回起動時に同じ状態に戻ります。

### 実装

| ファイル | 役割 |
|---|---|
| `Imervue/desktop_pet/pet_window.py` | トップレベルオーバーレイ — フレームレス / 常に最前面 / `WA_TranslucentBackground`。`PuppetCanvas(pet_mode=True)` をホストし、ドラッグして移動、ヒット検出、コンテキストメニュー、スピーチバブル統合、フルスクリーン検出器の配線、ドライバー、永続化ライトスルーを所有。 |
| `Imervue/desktop_pet/edge_snap.py` | 純粋 Python のスナップ計算(Qt なし)。ユニットテスト可能なコーナー / エッジドッキング + オーバーシュートクランプ。 |
| `Imervue/desktop_pet/settings.py` | 永続化ヘルパー — ロード / 保存 / 更新 / クランプ。 |
| `Imervue/desktop_pet/speech_bubble.py` | フレームレスの丸みのあるバブルオーバーレイ、矩形アンカー配置 + フェードアニメーション。 |
| `Imervue/desktop_pet/fullscreen_detector.py` | 前景ウィンドウの矩形を読み取る 1 Hz ポーリングループ(Windows では Win32 ctypes、それ以外では no-op フォールバック)、`state_changed(bool)` を emit。 |
| `Imervue/desktop_pet/pet_workspace.py` | コントロールパネルタブ。オーバーレイを遅延作成し、すべてのトグル / スライダー / コンボをチェックボックスまたはスピンボックスとして公開、最後にロードした rig + 起動時表示の動作を永続化。 |
| `Imervue/desktop_pet/tray_icon.py` | システムトレイヘルパー — セッション中シングルインスタンス、ワークスペースのチェック状態と同期。 |

`PuppetCanvas.__init__(pet_mode=True)` はエディタの透明チェッカー背景と選択オーバーレイを短絡し、それ以外のレンダーパス(メッシュ VBO、モーションプレイヤー、物理、表情、ポーズグループ)は Puppet タブと同一です。

すべての UI 文字列は `language_wrapper.language_word_dict.get(...)` 経由でルーティングされ、5 つのベース言語パック(English、繁體中文、简体中文、日本語、한국어)すべてでキーが定義されています。
...
- **現像(非破壊)** — トーンカーブ · .cube LUT · スプリットトーニング · 局所調整マスク · ソフトプルーフ
