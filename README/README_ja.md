...
**Modify** タブは現像ワークステーションです。すべての調整は画像ごとの **recipe** に保存され、明示的に **Export** または **Save As** を実行するまで、元ファイルは上書きされません。例外は **Apply Crop** と注釈の **Save** の 2 つで、結果を元ファイルに書き戻しますが、EXIF（カメラ、撮影日時、GPS）、XMP、DPI は保持されます。カメラ RAW、HEIC、アニメーション / マルチページのファイルは決して上書きされず、切り抜きはエクスポートを促し、注釈の保存は新しいファイル名を尋ねます。
