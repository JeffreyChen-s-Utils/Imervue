...
第五个标签 — **Desktop Pet** 把任意 `.puppet` rig 以无边框、透明、永远置顶的 overlay 跑在桌面上。标签本身是控制面板；实际角色另外开一个 top-level 窗口，跟 Puppet 共用同一条 runtime — 同样的 `PuppetCanvas`、同样的参数 / 动作 / 物理管线、同样的实时输入驱动。

### 窗口行为

| 项目 | 说明 |
|---|---|
| 无边框 overlay | 无窗口外壳、无 taskbar entry；通过 `Qt.WindowStaysOnTopHint` 永远置顶于所有其他窗口之上。 |
| 透明背景 | `WA_TranslucentBackground` + alpha-aware 的 GL surface format + `glClearColor(0,0,0,0)` — 角色没画到的像素全部让桌面透出来。 |
| 拖拽移动 | 左键拖角色可重新定位。释放时若距离屏幕边缘在可配置的吸附阈值（默认 24 px）以内，自动 **吸附** 紧贴边缘。快速拖出屏幕时会 clamp 回屏幕内，宠物不会跑到屏幕外抓不回来。 |
| 点击穿透 toggle | 可选的 `Qt.WindowTransparentForInput` 模式 — 所有点击穿过宠物传到后面的桌面 / 应用。 |
| 锚点锁定 | 一键冻结宠物位置，避免误拖移动。 |
| 永远置底 | 从 `WindowStaysOnTopHint` 切到 `WindowStaysOnBottomHint`，让宠物像桌面挂件一样位于所有窗口之后（搭配 `WindowDoesNotAcceptFocus`）。 |
| 全屏时自动隐藏 | 1 Hz 的轮询通过 Win32 `GetWindowRect` API（Windows）监视前台窗口，当其他应用在宠物所在屏幕全屏时自动隐藏宠物。 |
| 隐藏时暂停 | overlay 隐藏期间停掉 33 ms 的 paint tick，休眠中的宠物零 CPU 开销。`showEvent` 时恢复。 |
| 尺寸 preset | small（200×300）/ medium（320×480）/ large（480×720）三档；中心对齐缩放，调尺寸时角色不会跨屏幕跳。 |
| 不透明度滑杆 | 窗口级 opacity 0.1 – 1.0，通过 `setWindowOpacity` 控制 — WA_TranslucentBackground 合成加上每窗口 alpha 可达到平滑淡化，而不只是减暗角色像素。 |
| 位置持久化 | 每次拖动释放后吸附完的 `(x, y)` 会写入 `user_setting_dict["desktop_pet"]["position"]`。下次启动时宠物回到该屏幕位置；多显示器断开时回退到主屏幕右下角。 |

### 交互

| 项目 | 说明 |
|---|---|
| **左键点身体** | 通过 pan / zoom 矩阵的逆变换把点击映射回 puppet-canvas 坐标，跑现有的 `hit_test()` 比对 document 的 `HitArea` entry，若命中的 drawable 有挂动作就播放。无命中时在对话气泡里轮播一句问候作为 fallback。 |
| **右键任意处** | 弹出 context menu：Hide pet、**Live drivers** 子菜单（6 个 checkable toggle）、**Play motion** 子菜单（从 `document.motions` 填充）、**Apply expression** 子菜单（从 `document.expressions` 填充）、Lock position、Click-through、Always on bottom、Hide-on-fullscreen、Speech bubble toggle 以及 **Size** 子菜单。 |
| **对话气泡** | 无边框 / 透明 / 永远置顶的 QWidget，圆角气泡加尾巴。点击时浮现在宠物上方，停留约 4 s 后用 400 ms 淡出。锚定在宠物的 geometry，拖宠物时气泡跟着走。隐藏 / 换 rig 时自动消失。 |
| **系统托盘** | Show / Hide（checkable）、Click-through、Open puppet…、Hide pet。左键切换可见性；右键打开菜单。通过 `sync_visibility` / `sync_click_through` 与工作区的 check 状态保持同步。 |

### 实时驱动（lazy-init）

每个驱动在首次启用时才初始化，休眠中的宠物零 timer / thread 开销：

- **Auto idle** — 标准参数（`ParamBreath` 等）上的呼吸 + drift，由 `IdleDriver` 提供。
- **Idle motions** — 在 `Idle` group 的动作里随机循环，由 `IdleMotionCycler` + 内建 `MotionPlayer` 实现。
- **Auto-blink** — `ParamEyeLOpen` / `ParamEyeROpen` 上约 4.5 s 一次的 cosine 闭眼 - 睁眼曲线，由 `InputEngine.set_blink_enabled` 控制。
- **Drag-track head** — 光标偏移 → `ParamAngleX/Y` + `ParamEyeBallX/Y`，由 `InputEngine.set_drag_enabled` 控制。
- **Mic lip-sync** — 麦克风 RMS → `ParamMouthOpenY`，由 `InputEngine.set_lipsync_enabled` 控制（需要 `sounddevice`）。
- **Webcam tracking** — MediaPipe FaceLandmarker → 头 + 眼 + 嘴，由 `WebcamTracker` 提供（需要 `opencv-python` + `mediapipe`）。

### 持久化

`Imervue/desktop_pet/settings.py` 在 `user_setting_dict["desktop_pet"]` 之上加了一层封装：

- 每个 key 都有默认值，载入时做范围 clamp，settings 文件损坏也不会让启动崩溃。
- 单层深合并 — 旧版 settings 文件缺新 key 时仍能产生完整的 state dict。
- `drivers` 子 dict 做前向兼容 — 未知的 driver key 原样 round-trip，未来版本新增 driver 也能干净地读旧文件。

每个用户可调整的面（位置、尺寸、不透明度、点击穿透、锚点、置底、全屏隐藏、对话气泡、吸附阈值、各驱动、最近加载的 rig、启动时显示）都通过这个 helper round-trip，让宠物下次启动回到同一状态。

### 实现

| 文件 | 角色 |
|---|---|
| `Imervue/desktop_pet/pet_window.py` | top-level overlay — 无边框 / always-on-top / `WA_TranslucentBackground`。host `PuppetCanvas(pet_mode=True)`，自带拖拽移动、命中检测、context menu、对话气泡集成、全屏侦测连线、驱动、持久化 write-through。 |
| `Imervue/desktop_pet/edge_snap.py` | 纯 Python 的吸附算式（不依赖 Qt），便于单测的角落 / 边缘 docking + 越界 clamp。 |
| `Imervue/desktop_pet/settings.py` | 持久化 helper — load / save / update / clamp。 |
| `Imervue/desktop_pet/speech_bubble.py` | 无边框圆角气泡 overlay，带 anchor-to-rect 定位 + 淡出动画。 |
| `Imervue/desktop_pet/fullscreen_detector.py` | 1 Hz 轮询循环，读取前台窗口的 rect（Windows 上用 Win32 ctypes，其他平台 no-op fallback），并 emit `state_changed(bool)`。 |
| `Imervue/desktop_pet/pet_workspace.py` | 控制面板标签。lazy-create overlay，把每个 toggle / slider / combo 暴露成 checkbox 或 spinbox，持久化最近加载的 rig + show-on-launch 行为。 |
| `Imervue/desktop_pet/tray_icon.py` | 系统托盘 helper — 每个 session 单例，与工作区的 check 状态同步。 |

`PuppetCanvas.__init__(pet_mode=True)` 会跳过编辑器的透明棋盘格背景跟选择 overlay；其余渲染路径（mesh VBO、motion player、physics、expressions、pose group）跟 Puppet 标签一模一样。

每个 UI 字串都走 `language_wrapper.language_word_dict.get(...)`，对应的 key 在全部五个基础语言包（English、繁體中文、简体中文、日本語、한국어）都已定义。
