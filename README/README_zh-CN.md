...
第五个标签。**Desktop Pet** 把任意 `.puppet` rig 以无边框、透明背景、永远置顶的 overlay 跑在桌面上。标签本身是控制面板；实际角色另外开一个 top-level 窗口，跟 Puppet 标签共用同一条 runtime — 同样的 `PuppetCanvas`、同样的参数 / 动作 / 物理管线、同样的实时驱动。

### 功能

| 项目 | 说明 |
|---|---|
| 无边框 overlay | 无窗口外壳、无 taskbar entry；通过 `Qt.WindowStaysOnTopHint` 永远置顶。 |
| 透明背景 | `WA_TranslucentBackground` + alpha-aware 的 GL surface format + `glClearColor(0,0,0,0)` — 角色以外的像素全部让桌面透出来。 |
| 拖拽移动 | 左键拖角色身体可重新定位。释放时若距离屏幕边缘 ~24 px 以内自动 **吸附**。拖过头时自动 clamp 回屏幕内，宠物不会跑到屏幕外抓不回来。 |
| 点击穿透 | 可选的 `Qt.WindowTransparentForInput` 模式 — 所有点击穿过宠物传到后面的桌面 / 应用。宠物纯装饰时很实用。 |
| 系统托盘 | 显示 / 隐藏、点击穿透 toggle、Open puppet… — 左键点托盘 icon 切换显示。不用找到标签就能收掉宠物。 |
| 尺寸 preset | small / medium / large 三档；中心对齐缩放，调尺寸时角色不会跨屏幕跳。 |
| 实时驱动 | Auto idle（呼吸 + drift）、idle motion 随机循环、自动眨眼、头跟光标、麦克风对嘴、webcam 追踪 — Puppet 标签的全套 toggle。lazy-init，没开的功能不付 timer / thread 成本。 |

### 怎么启动

1. **标签 > Desktop Pet > Load bundled March 7th**（或 `Open Puppet…` 打开自己的 `.puppet`）。
2. 勾 **Show pet on desktop**，overlay 窗口就出现。
3. 挑想要的实时驱动；在 Puppet 标签录制或编辑的内容直接在这边播放。
4. 把角色拖到喜欢的角落、放开让它吸附。

### 实现

- `Imervue/desktop_pet/pet_window.py` — top-level overlay（无边框 / 透明 / always-on-top），host 一个 `pet_mode=True` 的 `PuppetCanvas`，转发驱动 toggle。
- `Imervue/desktop_pet/edge_snap.py` — 纯 Python 边缘吸附算式（不依赖 Qt），方便单测。
- `Imervue/desktop_pet/pet_workspace.py` — 控制面板标签。
- `Imervue/desktop_pet/tray_icon.py` — 系统托盘 helper。
