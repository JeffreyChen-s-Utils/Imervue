...
**Modify** 分頁是顯影工作站。每次調整都儲存在每張影像的 **recipe** 中 — 原始檔案直到你明確 **匯出** 或 **另存新檔** 才會被覆寫。例外是 **Apply Crop** 與註解的 **Save**：它們會把結果寫回原檔，並保留其 EXIF（相機、拍攝時間、GPS）、XMP 與 DPI。相機 RAW、HEIC 以及動畫／多頁檔案永遠不會被覆寫——裁切會提示改用匯出，註解儲存則會詢問新檔名。
