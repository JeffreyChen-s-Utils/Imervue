...
- **深度縮放金字塔** — 512×512 磁磚多層 LANCZOS 縮放；磁磚 LRU 保留 256 筆（硬上限 512）。VRAM 預算於啟動時向 GL 驅動探測，取得失敗則回落 1.5 GB，可用 `vram_limit_mb` 設定覆寫（會夾限，不會被靜默忽略）。最高 8× 各向異性過濾；遠超過 Pillow 安全上限（1.79 億像素）的全景圖也能開啟（上限依記憶體而定：16 GB 約 13 億像素）
...
- **被其他程式修改的檔案** — 其他程式覆寫存檔時（外部編輯器直接覆寫，或先寫副本再改名取代），檢視器會顯示新版本：深度縮放中開啟的圖片在最後一次寫入後 0.5 秒更新（Imervue 回到前景時也會再檢查），網格縮圖與清單的列在幾秒內更新
...
- **動畫播放** — GIF / APNG，含播放 / 暫停 / 逐格 / 速度控制；解碼後超過 512 MB 的動畫會邊播放邊逐格解碼，而不是一開始全部解碼；10 ms 以下的影格會和瀏覽器一樣顯示 100 ms；多頁 TIFF（掃描的文件）不會播放，而是一次顯示一頁，用 `,` 與 `.` 翻頁（顯示為「第 2/5 頁」）；相機嵌入 JPEG 的預覽（MPF）不會被當成第二個影格顯示
...
**Modify** 分頁是顯影工作站。每次調整都儲存在每張影像的 **recipe** 中 — 原始檔案直到你明確 **匯出** 或 **另存新檔** 才會被覆寫。例外是 **Apply Crop** 與註解的 **Save**：它們會把結果寫回原檔，並保留其 EXIF（相機、拍攝時間、GPS）、XMP 與 DPI。相機 RAW、HEIC 以及動畫／多頁檔案永遠不會被覆寫——裁切會提示改用匯出，註解儲存則會詢問新檔名。單次處理的工具（CLAHE、HSL 混色器、相框、自動拉直……）會把結果存成原圖旁的 `photo_clahe.png`；再執行一次會存成 `photo_clahe_1.png`，不會覆蓋上一次的結果。**依 EXIF 自動旋轉**、**EXIF 批次清除** 的副本與 **拆分頁面…** 也用同樣的方式編號。Imervue 無損旋轉照片（裁切框會跟著轉）或改寫其 EXIF（GPS 地理標記、EXIF 編輯器）時，照片的配方與虛擬副本都會跟著走；帶局部遮罩、圖層、鏡頭光暈或人臉標籤的配方則留在旋轉前的版本上，轉回來即可取回。
...
| Delete | 連同 `.xmp` / 註解 sidecar 移到資源回收筒（可復原）；在沒有資源回收筒的磁碟（記憶卡、USB 隨身碟、網路磁碟）上，檔案會留著，直到你確認永久刪除 |
| Escape | 結束深度縮放 / 全螢幕 |
...
| `levels_image` / `curve_image` | 黑白點＋gamma 色階；S 曲線／提亮陰影／壓縮高光的色調曲線 |
| `auto_color_balance_image` / `channel_mixer_image` | 自動白平衡（4 種方法）；3×3 通道混合器＋黑白轉換 |
