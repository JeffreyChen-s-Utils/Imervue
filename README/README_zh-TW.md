...
第五個分頁。**Desktop Pet** 把任意 `.puppet` rig 以無邊框、透明背景、永遠置頂的 overlay 跑在桌面上。分頁本身是控制面板；實際角色另外開一個 top-level 視窗，跟 Puppet 分頁共用同一條 runtime — 同樣的 `PuppetCanvas`、同樣的參數 / 動作 / 物理管線、同樣的即時輸入驅動。

### 視窗行為

| 項目 | 說明 |
|---|---|
| 無邊框 overlay | 沒有視窗外殼、沒有 taskbar entry；透過 `Qt.WindowStaysOnTopHint` 疊在所有其他視窗之上。 |
| 透明背景 | `WA_TranslucentBackground` + alpha-aware 的 GL surface format + `glClearColor(0,0,0,0)` — 角色沒畫到的每個像素都讓桌面透出來。 |
| 拖曳移動 | 左鍵拖角色身體可重新定位。釋放時若距離螢幕邊緣在可設定的吸附閾值內（預設 24 px）就自動 **吸附** 貼齊邊緣。快速拖過頭時 clamp 回螢幕內，寵物不會跑到螢幕外抓不回來。 |
| 點擊穿透 toggle | 可選的 `Qt.WindowTransparentForInput` 模式 — 所有點擊穿過寵物傳到後面的桌面 / 應用。 |
| 鎖定位置 | 一鍵凍結寵物位置，誤拖也動不了。 |
| 永遠置底 | 從 `WindowStaysOnTopHint` 翻成 `WindowStaysOnBottomHint`，讓寵物像桌面小工具一樣壓在所有視窗下方（搭配 `WindowDoesNotAcceptFocus`）。 |
| 全螢幕時自動隱藏 | 1 Hz 的 poller 透過 Win32 `GetWindowRect` API（Windows）監看當前作用視窗，當別的應用在寵物所在螢幕進入全螢幕時自動隱藏寵物。 |
| 隱藏時暫停 | overlay 隱藏期間停掉 33 ms 的繪製 tick，閒置寵物零 CPU；`showEvent` 時恢復。 |
| 尺寸 preset | 小（200×300）/ 中（320×480）/ 大（480×720）；以中心為錨點縮放，調尺寸時角色不會跨螢幕跳。 |
| 不透明度滑桿 | 透過 `setWindowOpacity` 設定視窗等級 0.1 – 1.0 的不透明度 — WA_TranslucentBackground 合成搭配 per-window alpha 帶來平滑淡入淡出，而不只是把角色像素變暗。 |
| 位置持久化 | 每次拖曳釋放後吸附完的 `(x, y)` 都寫進 `user_setting_dict["desktop_pet"]["position"]`。下次啟動寵物回到那個螢幕位置；多螢幕斷線時 fallback 到主螢幕右下角。 |

### 互動

| 項目 | 說明 |
|---|---|
| **左鍵點身體** | 透過 pan / zoom 反矩陣把點擊轉成 puppet-canvas 座標，對 document 的 `HitArea` 項目跑現成的 `hit_test()`，命中的 drawable 若有連結動作就播放。沒命中時 fallback 到對話泡泡裡輪流出現的問候語。 |
| **任意位置右鍵** | 開啟內容選單：隱藏寵物、**Live drivers** 子選單（6 個可勾選 toggle）、**Play motion** 子選單（由 `document.motions` 填入）、**Apply expression** 子選單（由 `document.expressions` 填入）、鎖定位置、點擊穿透、永遠置底、全螢幕時自動隱藏、對話泡泡 toggle、以及 **Size** 子選單。 |
| **對話泡泡** | 無邊框 / 透明 / 永遠置頂的 QWidget，圓角本體 + 小尾巴。點擊時從寵物上方彈出、停留約 4 秒、再以 400 ms 淡出。錨點綁在寵物 geometry，所以拖寵物時泡泡跟著走。寵物隱藏 / 換 rig 時自動消失。 |
| **系統托盤** | 顯示 / 隱藏（可勾選）、點擊穿透、Open puppet…、隱藏寵物。左鍵點 icon 切換顯示；右鍵開選單。透過 `sync_visibility` / `sync_click_through` 跟控制面板的勾選狀態同步。 |

### 即時驅動（lazy-init）

每個驅動都在第一次啟用時才建立，閒置寵物零 timer / thread 成本：

- **Auto idle** — 透過 `IdleDriver` 在標準參數（`ParamBreath` 等）上做呼吸 + drift。
- **Idle motions** — 透過 `IdleMotionCycler` + 內建的 `MotionPlayer` 隨機循環 `Idle` 群組裡的動作。
- **Auto-blink** — 透過 `InputEngine.set_blink_enabled` 在 `ParamEyeLOpen` / `ParamEyeROpen` 上每 ~4.5 秒跑一次 cosine 闔眼 - 開眼 cycle。
- **Drag-track head** — 透過 `InputEngine.set_drag_enabled` 把游標偏移映射到 `ParamAngleX/Y` + `ParamEyeBallX/Y`。
- **Mic lip-sync** — 透過 `InputEngine.set_lipsync_enabled` 把麥克風 RMS 映射到 `ParamMouthOpenY`（需要 `sounddevice`）。
- **Webcam tracking** — 透過 `WebcamTracker` 用 MediaPipe FaceLandmarker 驅動頭部 + 眼睛 + 嘴巴（需要 `opencv-python` + `mediapipe`）。

### 持久化

`Imervue/desktop_pet/settings.py` 在 `user_setting_dict["desktop_pet"]` 上做了一層包裝：

- 每個 key 都有預設值 + 載入時的範圍 clamp，settings 檔損毀也不會讓啟動 crash。
- 一層深度的 merge — 舊 settings 檔缺新 key 仍能還原成完整的 state dict。
- `drivers` 子 dict 的向前相容性 — 未知的 driver key 原樣 round-trip，未來版本新增 driver 也能乾淨讀取現有檔案。

每個使用者可調項（位置、尺寸、不透明度、點擊穿透、鎖定位置、永遠置底、全螢幕時自動隱藏、對話泡泡、吸附閾值、各驅動、上次載入的 rig、啟動時顯示）都透過這個 helper round-trip，寵物下次啟動回到相同狀態。

### 實作

| 檔案 | 角色 |
|---|---|
| `Imervue/desktop_pet/pet_window.py` | top-level overlay — 無邊框 / 永遠置頂 / `WA_TranslucentBackground`。host 一個 `PuppetCanvas(pet_mode=True)`，自帶拖曳移動、命中偵測、內容選單、對話泡泡整合、全螢幕偵測器接線、驅動、持久化 write-through。 |
| `Imervue/desktop_pet/edge_snap.py` | 純 Python 邊緣 / 角落吸附算式（不依賴 Qt），方便單測 + overshoot clamp。 |
| `Imervue/desktop_pet/settings.py` | 持久化 helper — load / save / update / clamp。 |
| `Imervue/desktop_pet/speech_bubble.py` | 無邊框圓角泡泡 overlay，依矩形錨點定位 + 淡出動畫。 |
| `Imervue/desktop_pet/fullscreen_detector.py` | 1 Hz poll loop 讀取前景視窗 rect（Windows 走 Win32 ctypes；其他平台 no-op fallback），發出 `state_changed(bool)`。 |
| `Imervue/desktop_pet/pet_workspace.py` | 控制面板分頁。lazy-create overlay，把每個 toggle / slider / combo 開成 checkbox 或 spinbox，持久化上次載入的 rig + 啟動時是否顯示。 |
| `Imervue/desktop_pet/tray_icon.py` | 系統托盤 helper — 每個 session 單 instance，跟控制面板勾選狀態同步。 |

`PuppetCanvas.__init__(pet_mode=True)` 會跳過編輯器的透明棋盤格背景跟選取 overlay；其餘渲染路徑（mesh VBO、motion player、physics、expressions、pose group）跟 Puppet 分頁一模一樣。

所有 UI 字串都走 `language_wrapper.language_word_dict.get(...)`，key 在五個基底語言包都有定義（English、繁體中文、简体中文、日本語、한국어）。
