...
第五個分頁。**Desktop Pet** 把任意 `.puppet` rig 以無邊框、透明背景、永遠置頂的 overlay 跑在桌面上。分頁本身是控制面板；實際角色另外開一個 top-level 視窗，跟 Puppet 分頁共用同一條 runtime — 同樣的 `PuppetCanvas`、同樣的參數 / 動作 / 物理管線、同樣的即時驅動。

### 功能

| 項目 | 說明 |
|---|---|
| 無邊框 overlay | 沒有視窗外殼、沒有 taskbar entry；透過 `Qt.WindowStaysOnTopHint` 永遠置頂。 |
| 透明背景 | `WA_TranslucentBackground` + alpha-aware 的 GL surface format + `glClearColor(0,0,0,0)` — 角色以外的像素全部讓桌面透出來。 |
| 拖曳移動 | 左鍵拖角色身體可重新定位。釋放時若距離螢幕邊緣 ~24 px 以內自動 **吝附**。拖過頭時自動 clamp 回螢幕內，寵物不會跑到螢幕外抓不回來。 |
| 點擊穿透 | 可選的 `Qt.WindowTransparentForInput` 模式 — 所有點擊穿過寵物傳到後面的桌面 / 應用。寵物純裝飾時很實用。 |
| 系統托盤 | 顯示 / 隱藏、點擊穿透 toggle、Open puppet… — 左鍵點托盤 icon 切換顯示。不用找到分頁就能收掉寵物。 |
| 尺寸 preset | small / medium / large 三檔；中心對齊縮放，調尺寸時角色不會跨螢幕跳。 |
| 即時驅動 | Auto idle（呼吸 + drift）、idle motion 隨機循環、自動眨眼、頭跟游標、麥克風對嘴、webcam 追蹤 — Puppet 分頁的全套 toggle。lazy-init，沒開的功能不付 timer / thread 成本。 |

### 怎麼啟動

1. **分頁 > Desktop Pet > Load bundled March 7th**（或 `Open Puppet…` 開自己的 `.puppet`）。
2. 勾 **Show pet on desktop**，overlay 視窗就出現。
3. 挑想要的即時驅動；在 Puppet 分頁錄製或編輯的內容直接在這邊播放。
4. 把角色拖到喜歡的角落、放開讓它吝附。

### 實作

- `Imervue/desktop_pet/pet_window.py` — top-level overlay（無邊框 / 透明 / always-on-top），host 一個 `pet_mode=True` 的 `PuppetCanvas`，轉發驅動 toggle。
- `Imervue/desktop_pet/edge_snap.py` — 純 Python 邊緣吝附算式（不依賴 Qt），方便單測。
- `Imervue/desktop_pet/pet_workspace.py` — 控制面板分頁。
- `Imervue/desktop_pet/tray_icon.py` — 系統托盤 helper。
