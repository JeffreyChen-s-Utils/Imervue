...
**Modify** 分頁是顯影工作站。每次調整都儲存在每張影像的 **recipe** 中 — 原始檔案直到你明確 **匯出** 或 **另存新檔** 才會被覆寫。例外是 **Apply Crop** 與註解的 **Save**：它們會把結果寫回原檔，並保留其 EXIF（相機、拍攝時間、GPS）、XMP 與 DPI。相機 RAW、HEIC 以及動畫／多頁檔案永遠不會被覆寫——裁切會提示改用匯出，註解儲存則會詢問新檔名。單次處理的工具（CLAHE、HSL 混色器、相框、自動拉直……）會把結果存成原圖旁的 `photo_clahe.png`；再執行一次會存成 `photo_clahe_1.png`，不會覆蓋上一次的結果。Imervue 無損旋轉照片（裁切框會跟著轉）或改寫其 EXIF（GPS 地理標記、EXIF 編輯器）時，照片的配方與虛擬副本都會跟著走；帶局部遮罩、圖層、鏡頭光暈或人臉標籤的配方則留在旋轉前的版本上，轉回來即可取回。
...
- **另存新檔 / 匯出** — PNG / JPEG / WebP / BMP / TIFF，有損格式提供品質滑桿；保留相機、鏡頭與拍攝時間的 EXIF，位置可選（**中繼資料**：全部／位置以外／無）
- **批次操作** — 重命名、移動 / 複製、旋轉選取影像。移動或複製不會覆蓋同名檔案（會以 `name_1` 存入）；在 Imervue 裡重新命名或移動的照片（批次重新命名、Token 批次重新命名、資料夾樹、移動 / 複製、雙窗格、暫存區、影像整理）會保留評等、收藏、標籤、顏色標籤、標題、備註與篩選標記，`.xmp` 與註解 sidecar 也會一起帶走；資料夾在 Imervue 中開著時，用其他程式重新命名的照片也一樣
