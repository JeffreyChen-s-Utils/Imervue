...
* global — fill every pixel that matches the seed within tolerance,
  regardless of connectivity. raster paint apps' "all matching pixels" mode.
...
* ``expand`` — after the contiguous / global mask is computed, dilate
  ...
  edge — raster paint apps' "Close Gap" / "縮放" slider.
...
def flood_fill(
    ...
    *,
    ...
    ``gap_close`` (0..``MAX_GAP_CLOSE``) is raster paint apps' "Color Drop"
...
def _build_candidate_mask(
    ...
    *,
    ...
    if gap_close_px > 0:
        ...
        # raster paint apps' "Close gap" slider.
