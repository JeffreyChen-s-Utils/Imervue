...
def _process_single_image(
    ...
    if mode == MODE_ANIME:
        boxes = _detect_regions_anime(src, confidence)
    else:
        boxes = _detect_regions_real(detector, src, confidence, labels)
...
class _SingleWorker(QThread):
    ...
    def run(self):
        try:
            ...
            detector = _get_detector() if self._mode != MODE_ANIME else None
...
def _build_mode_row(layout, lang) -> tuple[QComboBox, QDoubleSpinBox, QSpinBox]:
    """Add detection-mode combo + confidence spinner + expand% to *layout*.
    Returns (mode_combo, confidence_spin, expand_pct_spin).
    """
    ...
    expand_spin.setValue(_MODE_DEFAULTS[MODE_REAL]["expand_pct"])
    ...
    return mode_combo, conf_spin, expand_spin
...
class ScanAllDialog(QDialog):
    ...
    def _build_ui(self):
        ...
        # Mode + confidence
        self._mode_combo, self._conf_spin, self._expand_spin = _build_mode_row(
            layout, self._lang)
...
def _ensure_deps(parent, on_ready, mode: str = MODE_REAL):
    pkgs = REQUIRED_PACKAGES_ANIME if mode == MODE_ANIME else REQUIRED_PACKAGES_REAL
    try:
