...
> 產出日期：2026-08-03（全樹掃描）· 最後同步：2026-09-24 · 對應 commit `c8f3600` · 分支 `dev` · 版本 `1.0.90`
>
...
其中 `Imervue/` 套件本身 751 檔 / 160,605 行。
...
| `exif_sidebar.py` | 436 | 可收合的 EXIF 側邊欄（含星等元件） |
...
| `pip_installer.py` | 843 | 外掛相依安裝器：下載內嵌 Python、安裝 pip 套件（凍結環境亦可），每次安裝都帶 `pip_constraints` 的約束檔；再匯出 `python_finder` 的名稱（外掛依賴 `pip_installer._find_python`） |
...
823 個檔、134,650 行。`pyproject.toml` 定義三個互斥層級 marker：
