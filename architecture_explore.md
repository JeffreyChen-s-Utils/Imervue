...
> 產出日期：2026-08-03（全樹掃描）· 最後同步：2026-09-26 · 對應 commit `ca1674a` · 分支 `dev` · 版本 `1.0.90`
>
...
其中 `Imervue/` 套件本身 787 檔 / 169,254 行。
...
34 個檔、8,344 行。無邊框、透明、永遠置頂的桌面寵物懸浮視窗，**共用整個 Puppet 執行期**。
...
897 個檔、150,532 行。`pyproject.toml` 定義三個互斥層級 marker：
