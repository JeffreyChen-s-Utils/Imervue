...
> 產出日期：2026-08-03（全樹掃描）· 最後同步：2026-09-23 · 對應 commit `432b4e2` · 分支 `dev` · 版本 `1.0.90`
>
...
其中 `Imervue/` 套件本身 741 檔 / 160,006 行。
...
159 個檔、32,444 行 —— 全部是 Qt 前端。多數對話框只是外殼，數學在 `image/`。
...
189 個檔、45,927 行 —— 全樹最大的子系統，是一個完整的點陣繪圖 + 漫畫製作工作區。
...
33 個檔、8,195 行。無邊框、透明、永遠置頂的桌面寵物懸浮視窗，**共用整個 Puppet 執行期**。
...
791 個檔、129,572 行。`pyproject.toml` 定義三個互斥層級 marker：
