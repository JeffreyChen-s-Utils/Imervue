...
> 產出日期：2026-08-03（全樹掃描）· 最後同步：2026-09-22 · 對應 commit `2c6af4d` · 分支 `dev` · 版本 `1.0.90`
>
...
其中 `Imervue/` 套件本身 739 檔 / 159,836 行。
...
159 個檔、32,457 行 —— 全部是 Qt 前端。多數對話框只是外殼，數學在 `image/`。
...
`optimize_dialog.py`(112) 目標檔案大小 · `gif_video_dialog.py`(383) · `contact_sheet_dialog.py`(199) ·
...
189 個檔、45,922 行 —— 全樹最大的子系統，是一個完整的點陣繪圖 + 漫畫製作工作區。
...
57 個檔、15,214 行。2D 骨架人偶動畫，Live2D Cubism 相容。原本是外掛，因為核心路徑
...
773 個檔、127,300 行。`pyproject.toml` 定義三個互斥層級 marker：
