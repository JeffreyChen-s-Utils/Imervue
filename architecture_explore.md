...
> 產出日期：2026-08-03（全樹掃描）· 最後同步：2026-09-25 · 對應 commit `ce6eb40` · 分支 `dev` · 版本 `1.0.90`
>
...
| `Imervue/gui/` | 165 | 33,060 |
| `Imervue/puppet/` | 57 | 15,292 |
| `Imervue/image/` | 125 | 14,464 |
...
其中 `Imervue/` 套件本身 773 檔 / 166,138 行。
...
125 個模組、14,464 行，**只有 `info.py` import Qt**（用 `QMessageBox` 顯示圖片資訊對話框），其餘都可在 worker
...
165 個檔、33,060 行 —— 全部是 Qt 前端。多數對話框只是外殼，數學在 `image/`。
...
866 個檔、142,018 行。`pyproject.toml` 定義三個互斥層級 marker：
